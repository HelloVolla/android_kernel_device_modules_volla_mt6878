...
// drv add tankaikun, add step charging, start
#include "mtk_charger.h"
// drv add tankaikun, add step charging, end
...
/* prize liuyong, modify for shutdown voltage, 20230516, start */
#define UI_SOC0_SHUTDOWN_VOLTAGE 3425
/* prize liuyong, modify for shutdown voltage, 20230516, end */
...
#define USER_RSENSE             2000  /* mhom rsense * 1000  for convenience calculation */

#define queue_delayed_work_time  5000  //hjw for  8000->1000
#define queue_start_work_time    50

#define CW_SLEEP_20MS           20
#define CW_SLEEP_10MS           10
#define CW_UI_FULL              94
...
static int cw_get_capacity(struct cw_battery *cw_bat)
{
	...
	/* prize liuyong, modify for shutdown voltage, 20230516, start */
	//if ((ui_soc < 1) && (cw_bat->voltage >= UI_SOC0_SHUTDOWN_VOLTAGE)) {
	//		ui_soc = 1;
	//}
	if (ui_soc < 1) {
		temp_soc = 1;
		if (cw_bat->voltage >= UI_SOC0_SHUTDOWN_VOLTAGE) {
			count = 0;
		} else {
			count++;
		}
		if (count >= 5) {
			printk("%s:Low voltage power off\n", __func__);
		} else {
			ui_soc = temp_soc;
		}
	}
...
static int cw_pinctrl_init(struct cw_battery *cw_bat)
{
	struct device *dev = cw_bat->dev;
	int ret = 0;

	cw_bat->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(cw_bat->pinctrl)) {
		ret = PTR_ERR(cw_bat->pinctrl);
		dev_info(dev, "failed to get pinctrl, ret=%d\n", ret);
		return ret;
	}

	cw_bat->default_mode =
		pinctrl_lookup_state(cw_bat->pinctrl, "i2c0_default_mode");

	if (IS_ERR(cw_bat->default_mode)) {
		dev_info(dev, "Can *NOT* find default_mode\n");
		cw_bat->default_mode = NULL;
	} else
		dev_info(dev, "Find default_mode\n");

	if (cw_bat->default_mode)
		pinctrl_select_state(cw_bat->pinctrl, cw_bat->default_mode);
