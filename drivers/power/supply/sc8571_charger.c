            ...
            int channel, int *result)
{
	...
	if(channel == ADC_IBAT){
		if(!sc->cw_bat){
			...
			if(!sc->cw_bat){
				pr_err("gezi failed to get cw_bat too!\n");
				*result = 1;
				return 0;
			}
			else{
...
to:
			...
			*result = prop.intval;
			dev_info(sc->dev,"%s sh366003 ibat:%d", __func__, ibat);
			return 0;
		}
	}
...
__maybe_unused static int sc8571_init_device(struct sc8571_chip *sc)
{
    ...
    struct {
		...
		{IBUS_UCP_TH, sc->cfg.ibus_ucp_th},
...
static int mtk_sc8571_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	struct sc8571_chip *sc = charger_get_data(chg_dev);
#if IS_ENABLED(CONFIG_PRIZE_MT5728_SUPPORT_18W)	
	pr_err("%s:%d,revere_mode:%d\n",__func__,en,revere_mode);
#else
	pr_err("%s:%d\n",__func__,en);
#endif	
	if(en){
...
*/
		...
		if(revere_mode){
			...
			sc8571_field_write(sc, OTG_EN, 1);
			msleep(10);
			sc8571_field_write(sc, ACDRV1_EN, 1);
		}
...
*/		sc8571_field_write(sc, ACDRV1_EN, 0);
...
static int sc8571_parse_dt(struct sc8571_chip *sc, struct device *dev)
{
    ...
    struct {
		...
		{"sc,sc8571,ibus-ucp-th", &(sc->cfg.ibus_ucp_th)},
                    ...
                    const struct i2c_device_id *id)
{
	...
	if (IS_ERR_OR_NULL(sc->chg_dev)) {
		...
		dev_err(sc->dev,"register charger dev:%s success!\n",sc->chg_dev_name);
	}
#endif /*CONFIG_MTK_CLASS*/
