...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	//ctx->bl_gpio =
	//devm_gpiod_get(ctx->dev, "bl", GPIOD_OUT_HIGH);
	//if (IS_ERR(ctx->bl_gpio)) {
	//		dev_err(ctx->dev, "%s: cannot get bl_gpio %ld\n",
	//			__func__, PTR_ERR(ctx->bl_gpio));
	//	return PTR_ERR(ctx->bl_gpio);
	//}
	//gpiod_set_value(ctx->bl_gpio, 0);
	//devm_gpiod_put(ctx->dev, ctx->bl_gpio);
...
static struct drm_display_mode performance_mode_120 = {
	//.clock = 336440,		//htotal*vtotal*120/1000
	...
	.vtotal = VAC + VFP_120HZ + VSA + VBP, //2400+1291+10+10 = 3711
};

static struct drm_display_mode performance_mode_90 = {
	//.clock = 340798,		//htotal*vtotal*90/1000
				 ...
				 unsigned int level)
{
	...
	chargepump_set_backlight_level(reg_level);
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	//ctx->bl_gpio = devm_gpiod_get(dev, "bl", GPIOD_OUT_HIGH);
	//if (IS_ERR(ctx->bl_gpio)) {
	//	dev_info(dev, "cannot get bl-gpios %ld\n",
	//		 PTR_ERR(ctx->bl_gpio));
	//	return PTR_ERR(ctx->bl_gpio);
	//}
	//devm_gpiod_put(dev, ctx->bl_gpio);
...
/* pri LAX10-192 added by xuejian 20240409 begin*/
    ...
    strcpy(current_lcm_info.vendor,"qingchuanggao");
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "boe,ft8725,vdo-qcg-s600-dphy", },
	{}
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-boe-ft8725-vdo-qcg-s600-dphy",
		.owner = THIS_MODULE,
