...
#include "include/panel-rayle-nt37701-cmd-120hz.h"
...
static atomic_t get_current_fps;
...
static void lcm_panel_init(struct lcm *ctx)
{
	...
	lcm_dcs_write_seq_static(ctx,0xBE,0xD2,0x80,0x04);//code version
	...
	lcm_dcs_write_seq_static(ctx,0x6F,0x61);//ENTER AOD EMsetting
	lcm_dcs_write_seq_static(ctx,0xF3,0x00); //20240902 from 0x80 to 0x00
	lcm_dcs_write_seq_static(ctx,0x6F,0x1D);//FPR RCN code
...
//	lcm_dcs_write_seq_static(ctx,0xFB,0x00,0x00);
...
//	lcm_dcs_write_seq_static(ctx,0xFB,0x00,0x00);
	...
	lcm_dcs_write_seq_static(ctx,0x91,0x89,0x28,0x00,0x0C,0xC2,0x00,0x02,0x2C,0x01,0x2E,0x00,0x08,0x08,0xBB,0x07,0xA1,0x10,0xF0);
	lcm_dcs_write_seq_static(ctx,0x88,0x01);
	lcm_dcs_write_seq_static(ctx,0x6F,0x01);
	lcm_dcs_write_seq_static(ctx,0x88,0x02,0x58,0x09,0x68);
	lcm_dcs_write_seq_static(ctx,0x6F,0x05);
	lcm_dcs_write_seq_static(ctx,0x51,0x8F,0xFF);
	if (atomic_read(&get_current_fps) == 60) {
		lcm_dcs_write_seq_static(ctx,0x2F,0x01);//ENTER 60HZ
	} else if (atomic_read(&get_current_fps) == 120) {
		lcm_dcs_write_seq_static(ctx,0x2F,0x03 );//ENTER 120HZ
	} else {
		lcm_dcs_write_seq_static(ctx,0x2F,0x01);//ENTER 60HZ
	}
	...
	lcm_dcs_write_seq_static(ctx,0x2B,0x00,0x00,0x0A,0x5B);
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	pr_info("get_current_fps:%d\n", atomic_read(&get_current_fps));
	lcm_panel_init(ctx);
...
//	char bl_set_aod[] = {0x51, 0x0D, 0xBB, 0x07, 0xFF};

//	if (lcm_aod_status) {
//		pr_info("[doze] skip set lcm_setbacklight_cmdq\n");
//		cb(dsi, handle, bl_set_aod, ARRAY_SIZE(bl_set_aod));
//		return 0;
//	}
	...
	enum MTK_PANEL_MODE_SWITCH_STAGE stage)
{
	if (stage == BEFORE_DSI_POWERDOWN) {
		...
		ctx->current_fps = 120;
		atomic_set(&get_current_fps, 120);
	}
}
...
/* pri LAX10-192 added by xuejian 20240409 begin*/
    ...
    strcpy(current_lcm_info.vendor,"rayle");
    sprintf(current_lcm_info.id,"0x%02x",0xd2);
    strcpy(current_lcm_info.more,"1200*2652");
#endif
/* pri LAX10-192 added by xuejian 20240409 end*/
	ctx->hbm_mode = 0;
...
MODULE_DESCRIPTION("nt37701 rayle CMD Panel Driver");
MODULE_LICENSE("GPL");
