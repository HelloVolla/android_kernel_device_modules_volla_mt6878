...
extern struct sensor_info_t gc6133_info;
extern struct sensor_info_t gc2375h_info;
extern struct sensor_info_t bf2257cs_info;
	...
	if (!spc_data->is_enabled){
		...
		if (ret){
			...
			if (ret == 0xa310){
				...
				spc_data->ops = &gc6133_info;
				ret = spc_data->ops->open(client);
			}  else if (ret == 0x2375) {
				spc_data->ops = &gc2375h_info;
				ret = spc_data->ops->open(client);
			} else if (ret == 0x2257) {
				spc_data->ops = &bf2257cs_info;
				ret = spc_data->ops->open(client);
			}else{
				ret = -EINVAL;
			}
...
static DEVICE_ATTR(value, 0444, shutter_value_show, NULL);
...
static ssize_t device_type_show(struct device *dev, struct device_attribute *attr,char *buf)
{
	...
	switch(spc_data->sensor_type){
		...
		case SENSOR_TYPE_6133: ret = sprintf(buf,"%s\n","GC6133"); break;
		case SENSOR_TYPE_2375: ret = sprintf(buf,"%s\n","GC2375"); break;
		case SENSOR_TYPE_2257: ret = sprintf(buf,"%s\n","BF2257CS"); break;
...
static const struct of_device_id spc_of_match[] = {
...
static int spc_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id){
	...
	if (!IS_ERR(node)){
		...
		spc_data->avdd_pin = of_get_named_gpio(node,"vcama-gpio",0);
		printk("SPC spc_r_avdd_pin(%d)\n",spc_data->avdd_pin);
		gpio_request(spc_data->avdd_pin,client->name);
		gpio_direction_output(spc_data->avdd_pin,0);

		spc_data->iovdd_pin = of_get_named_gpio(node,"vcamio-gpio",0);
		printk("SPC spc_r_iovdd_pin(%d)\n",spc_data->iovdd_pin);
		gpio_request(spc_data->iovdd_pin,client->name);
	...
	if (!IS_ERR(node)){
		...
		if (ret){
			...
			}else if (sensor_type == 0x6133){
				spc_data->sensor_type = SENSOR_TYPE_6133;
				spc_data->ops = &gc6133_info;
			} else if (sensor_type == 0x2375) {
				spc_data->sensor_type = SENSOR_TYPE_2375;
				spc_data->ops = &gc2375h_info;
			} else if (sensor_type == 0x2257) {
				spc_data->sensor_type = SENSOR_TYPE_2257;
				spc_data->ops = &bf2257cs_info;
			} else {
				spc_data->sensor_type = SENSOR_TYPE_UNKNOWN;
				spc_data->ops = &gc0310_info;
...
void spc_i2c_remove(struct i2c_client *client) 
{
	struct spc_data_t *spc_data = i2c_get_clientdata(client);
...
static struct i2c_driver spc_driver = {
	.driver = {
		.name		= "spc",
		.owner		= THIS_MODULE,
		.of_match_table	= of_match_ptr(spc_of_match),
