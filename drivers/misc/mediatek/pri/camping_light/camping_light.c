...
static int g_pwm_num=3;//PWM3
// drv add red blue Warning lights yaozhipeng 20240517 start
static struct pinctrl_state *red_en_h;
static struct pinctrl_state *red_en_l;
static struct pinctrl_state *red_h;
static struct pinctrl_state *red_l;
static struct pinctrl_state *blue_h;
static struct pinctrl_state *blue_l;
struct class *camping_red_blue_class;
static int camping_red_blue_status=0;
static int red_state = 0;
static int blue_state = 0;
static int light_status = 0;
// drv add red blue Warning lights yaozhipeng 20240517 end
//
...
// drv add red blue Warning lights yaozhipeng 20240517 start
static int prize_camping_red_blue_set_enable(int led_mode)
{
	switch (led_mode) {
		case 0:
			printk("Warning lights >> blue and red off\n");
			red_state = 0;
			blue_state = 0;
			pinctrl_select_state(camping_light_pinctrl,red_l);
			pinctrl_select_state(camping_light_pinctrl,red_en_l);
			pinctrl_select_state(camping_light_pinctrl,blue_l);
			if (camping_light_status ==0){
				pinctrl_select_state(camping_light_pinctrl,led_en_l);
				printk("red blue camping close\n");
			}
			break;
		case 1:
			printk("Warning lights >> red open\n");
			red_state = 1;
			pinctrl_select_state(camping_light_pinctrl,led_en_h);
			pinctrl_select_state(camping_light_pinctrl,red_en_h);
			pinctrl_select_state(camping_light_pinctrl,red_h);		
			break;
		case 2:
			printk("Warning lights >> red close\n");
			red_state = 0;
			pinctrl_select_state(camping_light_pinctrl,red_l);
			pinctrl_select_state(camping_light_pinctrl,red_en_l);		
			if ((blue_state == 0) && (camping_light_status ==0)){
				pinctrl_select_state(camping_light_pinctrl,led_en_l);
				printk("red blue camping close\n");
			}		
			break;
		case 3:
			blue_state = 1;
			printk("Warning lights >> blue open\n");
			pinctrl_select_state(camping_light_pinctrl,led_en_h);
			pinctrl_select_state(camping_light_pinctrl,blue_h);	
			break;
		case 4:
			blue_state = 0;
			printk("Warning lights >> blue close\n");
			pinctrl_select_state(camping_light_pinctrl,blue_l);
			if ((red_state == 0) && (camping_light_status ==0)){
				pinctrl_select_state(camping_light_pinctrl,led_en_l);
				printk("red blue camping close\n");
			}
			break;
...
static ssize_t camping_red_blue_store(struct device *dev, struct device_attribute *attr,const char *buf, size_t size)
{
	if(sscanf(buf, "%u", &camping_red_blue_status) != 1)
	{
		camping_light_log("[Warning lights]: Invalid values\n");
		return -EINVAL;
	}

	camping_light_log("[Warning lights] %s camping_red_blue_status value = %d [0:OFF ; mode 1~2 ; 1:red 2:blue ]\n ", __func__, camping_red_blue_status);
	if(camping_red_blue_status >= 0)
	{
		prize_camping_red_blue_set_enable(camping_red_blue_status);
	}
	else
	{
		camping_light_log("[Warning lights]: Invalid values\n");
...
static ssize_t camping_red_blue_show(struct device *dev, struct device_attribute *attr,char *buf)
{
    return sprintf(buf, "%d\n", camping_red_blue_status);
}
static DEVICE_ATTR(camping_red_blue, 0664, camping_red_blue_show, camping_red_blue_store);
// drv add red blue Warning lights yaozhipeng 20240517 end
...
int camping_light_dts(struct platform_device *pdev)
{
	...
	if (!IS_ERR(camping_light_pinctrl)){
		...
		if (IS_ERR(led_default)){
			printk("camping_light get pinctrl state led_default fail\n");
		}
		led_led_l = pinctrl_lookup_state(camping_light_pinctrl,"led_led_l");
		if (IS_ERR(led_led_l)){
			printk("camping_light get pinctrl state led_led_l fail\n");
		}
		led_led_set = pinctrl_lookup_state(camping_light_pinctrl,"led_led_set");
		if (IS_ERR(led_led_set)){
			printk("camping_light get pinctrl state led_led_set fail\n");
		}
		
		led_en_h = pinctrl_lookup_state(camping_light_pinctrl,"led_en_h");
		if (IS_ERR(led_en_h)){
			printk("camping_light get pinctrl state led_en_h fail\n");
		}
		led_en_l = pinctrl_lookup_state(camping_light_pinctrl,"led_en_l");
		if (IS_ERR(led_en_l)){
			printk("camping_light get pinctrl state led_en_l fail\n");
		}
// drv add red blue Warning lights yaozhipeng 20240517 start		
		red_en_h = pinctrl_lookup_state(camping_light_pinctrl,"red_en_h");
		if (IS_ERR(red_en_h)){
			printk("camping_light get pinctrl state red_en_h fail\n");
		}
		red_en_l = pinctrl_lookup_state(camping_light_pinctrl,"red_en_l");
		if (IS_ERR(red_en_l)){
			printk("camping_light get pinctrl state red_en_l fail\n");
		}
		red_h = pinctrl_lookup_state(camping_light_pinctrl,"red_h");
		if (IS_ERR(red_h)){
			printk("camping_light get pinctrl state red_h fail\n");
		}
		red_l = pinctrl_lookup_state(camping_light_pinctrl,"red_l");
		if (IS_ERR(red_l)){
			printk("camping_light get pinctrl state red_l fail\n");
		}
		blue_h = pinctrl_lookup_state(camping_light_pinctrl,"blue_h");
		if (IS_ERR(blue_h)){
			printk("camping_light get pinctrl state blue_h fail\n");
		}
		blue_l = pinctrl_lookup_state(camping_light_pinctrl,"blue_l");
		if (IS_ERR(blue_l)){
			printk("camping_light get pinctrl state blue_l fail\n");
		}
// drv add red blue Warning lights yaozhipeng 20240517 end		
	}else{
		printk("camping_light get pinctrl fail\n");
		return -EINVAL;
	}
...
static int camping_light_probe(struct platform_device *pdev)
{
	...
	struct device *camping_red_blue_dev;   // drv add red blue Warning lights yaozhipeng 20240517
    camping_light_log("%s\n", __func__);
...
// drv add red blue Warning lights yaozhipeng 20240517 start	
    camping_red_blue_class = class_create(THIS_MODULE, "camping_red_blue");
	if (IS_ERR(camping_red_blue_class)) {
		camping_light_log("Failed to create class(camping_red_blue_class)!");
		return PTR_ERR(camping_red_blue_class);
	}
	
	camping_red_blue_dev = device_create(camping_red_blue_class, NULL, 0, NULL, "camping_red_blue_data");
	if (IS_ERR(camping_red_blue_dev))
		camping_light_log("Failed to create camping_red_blue_dev device");
	if (device_create_file(camping_red_blue_dev, &dev_attr_camping_red_blue) < 0)
		camping_light_log("Failed to create device file(%s)!",dev_attr_camping_red_blue.attr.name);	
// drv add red blue Warning lights yaozhipeng 20240517 end
