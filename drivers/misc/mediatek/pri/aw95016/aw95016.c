...
#define AW95016_DRIVER_VERSION	"V0.1.0"

struct aw95016 *g_aw95016 = NULL;
static bool aw95016_probe_state = false;
...
static void aw95016_set_port_direction_by_mask(struct aw95016 *aw95016, unsigned int mask, int val)
{
	unsigned char reg_val[2] = {0};
	int i = 0;
...
static void aw95016_enbale_interrupt_by_mask(struct aw95016 *aw95016, unsigned int mask, unsigned int val)
{
	unsigned char reg_val[2] = {0};
	int i = 0;
...
static int aw95016_reset_gpio_dt(struct aw95016 *aw95016)
{
	int ret = 0;
	struct device_node *node = aw95016->dev->of_node;

	aw95016->rst_gpio = of_get_named_gpio(node, "reset-gpio", 0);

	if ((!gpio_is_valid(aw95016->rst_gpio))) {
		dev_err(aw95016->dev, "%s: dts don't provide reset-gpio\n", __func__);
		return -EINVAL;
	}
...
static int aw95016_irq_register(struct aw95016 *aw95016)
{
	...
	aw95016->irq_gpio = of_get_named_gpio(np, "irq-gpio", 0);
...
static void aw95016_key_chip_init(struct aw95016_key *p_key_data)
{
	...
	aw95016_set_port_output_mode(aw95016, 1); /* set output port pull push mode */
 ...
 ******************************************************/
 /* gpio sys node */
/* echo gpio_idx dirction state > awgpio */
static ssize_t awgpio_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
...
static ssize_t awgpio_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf+len, PAGE_SIZE-len, "Uasge:echo gpio_idx dirction state > awgpio\n");
...
void pri_aw95016a_gpio_set(enum aw95016_gpio_idx gpio_idx,int dir_io,int value_level)
{
	...
	struct aw95016_singel_gpio *p_single_gpio_data;
	
    if (!aw95016_probe_state)
        return;
    
    mutex_lock(&cmd_lock);
    p_gpio_data = g_aw95016->gpio_data;
	p_single_gpio_data = p_gpio_data->single_gpio_data;
    
    AW_DEBUG("aw95016_gpio_set: %d %d %d\n", gpio_idx, dir_io, value_level);

	if (p_gpio_data->gpio_mask & (0x01 << gpio_idx)) {
		for (i = 0; i < p_gpio_data->gpio_num; i++) {
			if (p_single_gpio_data[i].gpio_idx == gpio_idx) {
				//if (p_single_gpio_data[i].gpio_direction != dir_io) {
					p_single_gpio_data[i].gpio_direction = dir_io;
					aw95016_set_port_direction_by_mask(g_aw95016, 0x1 << p_single_gpio_data[i].gpio_idx,p_single_gpio_data[i].gpio_direction);
				//}
				if (p_single_gpio_data[i].gpio_direction == 0x01) { /* output */
					//if (p_single_gpio_data[i].state != value_level) {
						p_single_gpio_data[i].state = value_level;
						aw95016_set_port_output_state_by_mask(g_aw95016,0x1 << p_single_gpio_data[i].gpio_idx,p_single_gpio_data[i].state);
					//}
				}
			}
		}
...
EXPORT_SYMBOL(pri_aw95016a_gpio_set);
...
static int aw95016_key_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = aw95016_reset_gpio_dt(aw95016);
	if (ret)
		goto err_free_mem;
	...
	g_aw95016 = aw95016;//drv hjw
	
    mutex_init(&cmd_lock);
    aw95016_probe_state = true;
	AW_DEBUG("%s success!\n", __func__);
	return 0;
free_key:
...
static void aw95016_key_i2c_remove(struct i2c_client *client)
{
...
MODULE_AUTHOR("<wangshuaijie@awinic.com>");
