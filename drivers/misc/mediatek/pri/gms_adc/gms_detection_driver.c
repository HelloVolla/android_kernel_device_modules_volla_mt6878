...
static int gms_detection_probe(struct platform_device *pdev)
{
   ...
   printk("gms_detection_probe start\n");
	...
	if(ret < 0){
		printk(" lpp fail to get auxadc gms_adc_vol_max failed\n");
	}else{
		printk("lpp  get auxadc gms_adc_vol_max = %d\n",gms_adc_vol_max_value);
	}
	ret = of_property_read_u32(np,"gms_adc_vol_min",&gms_adc_vol_min_value);
	if(ret < 0){
		printk(" lpp fail to get auxadc gms_adc_vol_min failed\n");
	}else{
		printk("lpp  get auxadc gms_adc_vol_min = %d\n",gms_adc_vol_min_value);
	}	
	ret = of_property_read_u32(np,"mass_adc_vol_max",&mass_adc_vol_max_value);
	if(ret < 0){
		printk("lpp fail to get auxadc mass_adc_vol_max failed\n");
	}else{
		printk("lpp  get auxadc mass_adc_vol_max = %d\n",mass_adc_vol_max_value);
	}
	ret = of_property_read_u32(np,"mass_adc_vol_min",&mass_adc_vol_min_value);
	if(ret < 0){
		printk("lpp fail to get auxadc mass_adc_vol_min failed\n");
	}else{
		printk("lpp  get auxadc mass_adc_vol_min = %d\n",mass_adc_vol_min_value);
	}
	
	ret = of_property_read_u32(np,"reserve_adc_vol_max",&reserve_adc_vol_max_value);
	if(ret < 0){
		printk("lpp fail to get auxadc reserve_adc_vol_max failed\n");
	}else{
		printk("lpp  get auxadc reserve_adc_vol_max = %d\n",reserve_adc_vol_max_value);
	}
	ret = of_property_read_u32(np,"reserve_adc_vol_min",&mass_adc_vol_min_value);
	if(ret < 0){
		printk("lpp fail to get auxadc reserve_adc_vol_min failed\n");
	}else{
		printk("lpp  get auxadc reserve_adc_vol_min = %d\n",reserve_adc_vol_min_value);
	}
