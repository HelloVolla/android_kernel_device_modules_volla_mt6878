...
struct aw2016_led {
	...
	struct pinctrl_state *default_mode;
	struct device *dev;
};

static struct aw2016_led * g_aw2016_led = NULL;
...
static ssize_t gps_lna_store(struct class *class, struct class_attribute *attr,	const char *buf, size_t count)
{
	if(buf[0] == '0')
	{
		aw2016_set_brightness(&(g_aw2016_led[0].cdev),125);
	}
	else if(buf[0] == '1')
	{
		aw2016_set_brightness(&g_aw2016_led[0].cdev,0);
	}
	else if(buf[0] == '2')
	{
		aw2016_set_brightness(&g_aw2016_led[1].cdev,125);
	}
	else if(buf[0] == '3')
	{
		aw2016_set_brightness(&g_aw2016_led[1].cdev,0);
	}
	else if(buf[0] == '4')
	{
		aw2016_set_brightness(&g_aw2016_led[2].cdev,125);
	}
	else if(buf[0] == '5')
	{
		aw2016_set_brightness(&g_aw2016_led[2].cdev,0);
	}
	return count;
}
...
static struct class_attribute hd8040_class_attrs[] = {
	__ATTR(test, S_IRUGO | S_IWUSR, NULL, gps_lna_store),
	__ATTR_NULL,
};
...
static unsigned char aw2016_init_done = 0;

int charge_state_get_uisoc(void)
{
	...
	if (battery_psy == NULL || IS_ERR(battery_psy)) {
		...
		pr_err("%s gezi Couldn't get bat_psy\n", __func__);
		ret = -1;
	} else {
		ret = power_supply_get_property(battery_psy,POWER_SUPPLY_PROP_CAPACITY, &prop);
		ret = prop.intval;
	}
...
void charge_set_led_state(int mode)
{
	int ui_soc = 0;
	...
	if (boot_mode != 8 && boot_mode != 9) {
		pr_err("gezi not power off charge,boot_mode = %d\n",boot_mode);
		return;
	}
	
	if(!aw2016_init_done)
	{
		pr_err("gezi aw2016_init_done not init ok,%d\n",aw2016_init_done);
		return;
	}
	
	ui_soc = charge_state_get_uisoc();
	
	if(ui_soc == -1)
	{
		pr_err("gezi ui_soc not init ok\n");
		return;
	}
	
	//pr_err("gezi ui_soc = %d\n",pr_err("gezi ui_soc not init ok\n"););
	
	if((ui_soc >= 15) && (ui_soc <= 90))
	{
		blue_led_on();
	}
	else if(ui_soc > 90)
...
EXPORT_SYMBOL(charge_set_led_state);

static int hd8040_sysfs_create(void)
{
	int i = 0,ret = 0;
	
	hd8040_class = class_create(THIS_MODULE, "gezi");
	...
	return ret;
	//hd8040_class->dev_groups = rt5509_cal_groups;
}

static int aw2016_disable_auto_charge_indication(struct aw2016_led *led)
{
	int ret = 0;
	u8 val = 0;
	ret = aw2016_read(led, AW2016_REG_GCR1, &val);
	...
	val |= AW2016_CHARGE_DISABLE_MASK;
	
	ret = aw2016_write(led, AW2016_REG_GCR1, val);
...
/*
static int aw2016_pinctrl_init(struct aw2016_led *led)
{
	...
	if (IS_ERR(led->pinctrl)) {
		ret = PTR_ERR(led->pinctrl);
		dev_info(dev, "failed to get pinctrl, ret=%d\n", ret);
		return ret;
	}

	led->default_mode =
		pinctrl_lookup_state(led->pinctrl, "i2c3_default_mode");

	if (IS_ERR(led->default_mode)) {
		dev_info(dev, "Can *NOT* find default_mode\n");
		led->default_mode = NULL;
	} else
		dev_info(dev, "Find default_mode\n");

	if (led->default_mode)
		pinctrl_select_state(led->pinctrl, led->default_mode);
                ...
                const struct i2c_device_id *id)
{
	...
	//drv hjw for led otg leds start
	aw2016_disable_auto_charge_indication(led_array);
	//drv hjw for led otg leds end
	g_aw2016_led = led_array;
	
	battery_psy = power_supply_get_by_name("battery");
	
	if(battery_psy == NULL)
	{
		pr_err("gezi ----%s: get battery_psy power supply failed\n",__func__);
		//ret = -EINVAL;
		//goto err_no_psy;
	}
