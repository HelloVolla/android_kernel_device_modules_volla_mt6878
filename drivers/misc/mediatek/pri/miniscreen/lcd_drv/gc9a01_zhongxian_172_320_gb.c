...
void lcd_gc9a01_set_window(uint16_t xStart, uint16_t yStart, uint16_t xEnd, uint16_t yEnd, uint16_t xOffset, uint16_t yOffset)
{
	...
	pr_err("%s %d xstart = %d xend = %d ystart = %d yend = %d\n", __func__, __LINE__, xStart, xEnd, yStart, yEnd);
...
#define MINISCREEN_WIDTH     (320)
#define MINISCREEN_HEIGHT    (172)
...
void gc9a01_init(void)
{
	...
	gc9a01_write_data(0x05);   //0x05 65k,0x06 262k

	gc9a01_write_cmd(0xB2);
	gc9a01_write_data(0x0C);
	gc9a01_write_data(0x0C);
	gc9a01_write_data(0x00);
	gc9a01_write_data(0x33);
	gc9a01_write_data(0x33);

	gc9a01_write_cmd(0xB7);
	gc9a01_write_data(0x17);

	gc9a01_write_cmd(0xBB);
	gc9a01_write_data(0x35);   //0x26跳VCOM，抖动20211012

	gc9a01_write_cmd(0xC0);
	gc9a01_write_data(0x2C);

	gc9a01_write_cmd(0xC2);
	gc9a01_write_data(0x01);

	gc9a01_write_cmd(0xC3);
	gc9a01_write_data(0x13);   //对比度

	gc9a01_write_cmd(0xC4);
	gc9a01_write_data(0x20);   //VDV, 0x20:0v

	gc9a01_write_cmd(0xC6);
	gc9a01_write_data(0x18);  //0x1E
 
	gc9a01_write_cmd(0xD0);
	gc9a01_write_data(0xA7);
	...
	gc9a01_write_cmd(0xD6);
	gc9a01_write_data(0xA1);   //sleep in后，gate输出为GND

	gc9a01_write_cmd(0xE0);
	...
	gc9a01_write_cmd(0x35);   //打开TE
	gc9a01_write_data(0x00);

	gc9a01_write_cmd(0x44);     //移动撕裂线
	gc9a01_write_data(0x00);   //0x01&0x28
	...
	lcd_gc9a01_set_window(0, 0, MINISCREEN_WIDTH - 1, MINISCREEN_HEIGHT - 1, 0, 0x22);
}

static uint16_t r[MINISCREEN_WIDTH] = {0};
static uint16_t g[MINISCREEN_WIDTH] = {0};
static uint16_t b[MINISCREEN_WIDTH] = {0};
