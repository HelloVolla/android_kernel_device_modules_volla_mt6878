...
//struct tasklet_struct hall_tasklet;

static void hall_eint_handler(struct work_struct *data);
//static DECLARE_TASKLET(hall_tasklet, hall_eint_handler);
...
static ssize_t hall_status_info_show(struct kobject *kobj,
...
static ssize_t hall_status_info_store(struct kobject *kobj,
					struct kobj_attribute *attr,
...
//static DEVICE_ATTR(hall_status, S_IRUGO|S_IWUSR|S_IWGRP, hall_status_info_show,  hall_status_info_store);
static const struct kobj_attribute hall_status_attr =
	__ATTR(hall_status, 0664, hall_status_info_show,
		hall_status_info_store);
...
//prize modified by huarui, update with kernel version 20200408 start
	...
	hall_key_lock = wakeup_source_register(NULL, "hall key wakelock");
#else
	wake_lock_init(hall_key_lock, WAKE_LOCK_SUSPEND, "hall key wakelock");
#endif
//prize modified by huarui, update with kernel version 20200408 end
  INIT_WORK(&timer_work, hall_eint_handler);
   ...
   if(g_hall_first)
   {
	...
	ret = sysfs_create_file(g_hall_sys_device, &hall_status_attr.attr);
	if (ret) 
	{
