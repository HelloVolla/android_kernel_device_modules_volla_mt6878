...
#endif
static int cs_mode_status = 0;
static int cs_mode_result = 0;
typedef struct 
{
    short int reg_event_action;//0
    short int reg_event_area;
    short int reg_event_level;
    short int curPos;
    short int total_force;
    short int relative_mileage;
    short int oriStaPos;
    short int mode_type;
    short int fw_cnt;
    short int chip_temperature;

    short int fw_version;//10
    short int ch_tonglu;
    short int slide_tri_thr;
    short int slide_rel_thr;
    short int press_tri_thr1;
    short int press_rel_thr1;
    short int press_tri_thr2;
    short int press_rel_thr2;
    short int rawdata1;
    short int rawdata2;

    short int baseline1;//20
    short int baseline2;
    short int force_1;
    short int force_2;
    short int alg_press_status;
    short int alg_slide_status;
    short int alg_force_flag;
    short int alg_offset_flag;
    short int moto_trig_info;
    short int alg_force_speed;

    short int alg_distance_speed;//30
    short int alg_stapos;
    short int alg_mileage_thr;
    short int crc;
...
typedef union
{
    touch_sub_data_t sub;
    unsigned char r_buf[68];
}touch_data_t;

short Tool_CalcCheckSumS16(unsigned char* buff, unsigned short len)
{
    short int i,sum = 0;
    short int *p_cal_sum;
...
*return 0 - ok
*/
short get_pos_val(touch_data_t *touch_data)
{
    unsigned short len = 68;        //68个字节
    unsigned char addr = 0x20;      //0x20寄存器
    //unsigned char r_buf[68] = {0};  //read buff  
    short calc_sum = 0;             //计算校验和
    short check_sum = 0;            //协议校验和
//    short pos_val = 0;              //计算滑动值

    cs_press_iic_read(addr, touch_data->r_buf, len);

     check_sum = ((short)(touch_data->r_buf[len-1] << 8)) | touch_data->r_buf[len-2] ;
    // LOG_INFO("check_sum:0x%04X\n", check_sum);

    check_sum = 0 - check_sum;
    //LOG_INFO("0 - check_sum:0x%04X\n", check_sum);

    /* 校验 */
    calc_sum = Tool_CalcCheckSumS16(touch_data->r_buf, len-2);
    if(check_sum != calc_sum) {
        LOG_INFO("check_sum:0x%04x, calc_sum:0x%04x\n", check_sum, calc_sum);
        return -1;
    } else {
        //LOG_INFO("check_sum:success!\n");
    }
    /* 得出pos val */
    //pos_val = ((short)(r_buf[7] << 8)) | r_buf[6];
    //LOG_INFO("pos_val:%d\n", pos_val);
    return 0;
}
...
void fml_input_dev_init(void){
    ...
    if (cs_input_dev != NULL) {
		...
		__set_bit(KEY_PRESS_DOWM, cs_input_dev->keybit);
//		__set_bit(KEY_PRESS_KEEP, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_SLIDE_DOWM, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_SLIDE_UP, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_SLIDE_1, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_SLIDE_2, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_SLIDE_3, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_SLIDE_4, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_SLIDE_5, cs_input_dev->keybit);
		__set_bit(KEY_PRESS_WEIGHT, cs_input_dev->keybit);
...
#define DAILY_MODE  0x00
#define PROMPT_MODE 0x01
#define CAMERA_MODE 0x02
...
/**
  ...
  * @retval 0-ok ,-1 fail
*/
short config_FT_run_mode(unsigned char run_mode)
{
    ...
    do{
        retry--;
        data_buf[0] = run_mode;
        ret = cs_press_iic_write(0x55, data_buf, 1);
        if(ret != 0)
        {
            continue;
        }
        data_buf[0] = 0x55;
        data_buf[1] = 0xAB;
        ret = cs_press_iic_write(0x01, data_buf, 2);
        if(ret != 0)
        {
            continue;
        }
        ...
        for(i = 0; i < 15; i++)
        {
            data_buf[0] = 0xFF;
            data_buf[1] = 0xFF;
            ret = cs_press_iic_read(0x01, data_buf, 2);
            if(ret != 0)
            {
                continue;
            }
            ...
            {
				LOG_INFO("run_mode ret = %d\n", ret);
                cs_press_delay_ms(5);
                continue;
            }

            data_buf[0] = 0xFF;
            ret = cs_press_iic_read(0x55, data_buf, 1);
			LOG_INFO("set run mode %d data_buf[0] = %d  data_buf[1] = %d\n", run_mode, data_buf[0], data_buf[1]);
            if((ret == 0)&&(data_buf[0] == run_mode))
            {
				cs_mode_result = cs_mode_status;
				LOG_INFO("set run mode %d pass\n", run_mode);
                return 0;
            }
        }
    }while(retry > 0);

    LOG_INFO("set run mode %d fail\n", run_mode);
    return -1;
}
/**
...
void fml_key_report(void)
{
	...
	short int mileage;
    if(get_pos_val(&touch_data) == 0)
    {
        LOG_INFO("action %04X, area %04X, level %04X,\n", touch_data.sub.reg_event_action, touch_data.sub.reg_event_area, touch_data.sub.reg_event_level);

        LOG_INFO("curPos %d\n", touch_data.sub.curPos);
        LOG_INFO("total_force %d\n", touch_data.sub.total_force);
        LOG_INFO("relative_mileage %d\n", touch_data.sub.relative_mileage);
        LOG_INFO("oriStaPos %d\n", touch_data.sub.oriStaPos);
#if 1
        //动作:action; 区域:area; 等级:level
        action = touch_data.sub.reg_event_action;
        area = touch_data.sub.reg_event_area;
        level = touch_data.sub.reg_event_level;
		mileage = touch_data.sub.relative_mileage;

        if((action&0x0100)&&(area&0x0001)){
			LOG_INFO("action down slide mileage = %d\n", mileage);//下滑
            input_report_key(cs_input_dev, KEY_PRESS_SLIDE_DOWM, 1);
            input_sync(cs_input_dev);
            input_report_key(cs_input_dev, KEY_PRESS_SLIDE_DOWM, 0);
            input_sync(cs_input_dev);
			if(mileage == 1 || mileage == -1){
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_1, 1);
                input_sync(cs_input_dev);
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_1, 0);
                input_sync(cs_input_dev);
			}
			
			if(mileage == 2 || mileage == -2){
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_2, 1);
                input_sync(cs_input_dev);
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_2, 0);
                input_sync(cs_input_dev);
			}
			if(mileage == 3 || mileage == -3){
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_3, 1);
                input_sync(cs_input_dev);
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_3, 0);
                input_sync(cs_input_dev);
			}
			if(mileage == 4 || mileage == -4){
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_4, 1);
                input_sync(cs_input_dev);
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_4, 0);
                input_sync(cs_input_dev);
			}
			if(mileage == 5 || mileage == -5){
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_5, 1);
                input_sync(cs_input_dev);
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_5, 0);
                input_sync(cs_input_dev);
			}
        }
        if((action&0x0200)&&(area&0x0001)){
			LOG_INFO("action up slide mileage = %d\n", mileage);//上滑
            input_report_key(cs_input_dev, KEY_PRESS_SLIDE_UP, 1);
            input_sync(cs_input_dev);
            input_report_key(cs_input_dev, KEY_PRESS_SLIDE_UP, 0);
            input_sync(cs_input_dev);
			if(mileage == 1 || mileage == -1){
                input_report_key(cs_input_dev, KEY_PRESS_SLIDE_1, 1);
                input_sync(cs_input_dev);
        ...
        if((action&0x0020)&&(area&0x0001)&&(level&0x0004)){
			LOG_INFO("action double tap\n");//重双击
        }
        if((action&0x0001)&&(area&0x0001)&&(level&0x0001)){
			LOG_INFO("action down touch\n");//触摸dowm
            input_report_key(cs_input_dev, KEY_PRESS_DOWM, 1);
            input_sync(cs_input_dev);
        }
        if((action&0x0002)&&(area&0x0001)&&(level&0x0001)){
			LOG_INFO("action up touch\n");//触摸up
            input_report_key(cs_input_dev, KEY_PRESS_DOWM, 0);
            input_sync(cs_input_dev);
        }
        if((action&0x0008)&&(area&0x0001)&&(level&0x0100)){
			LOG_INFO("action keep touch\n");//触摸保持
        }
        if((action&0x0001)&&(area&0x0001)&&(level&0x0004)){
			LOG_INFO("action dowm W\n");//重按dowm
            input_report_key(cs_input_dev, KEY_PRESS_WEIGHT, 1);
            input_sync(cs_input_dev);
        }
        if((action&0x0002)&&(area&0x0001)&&(level&0x0004)){
		    LOG_INFO("action UP W\n");//重按 UP
            input_report_key(cs_input_dev, KEY_PRESS_WEIGHT, 0);
            input_sync(cs_input_dev);
        }
#endif
        //input_report_abs(cs_input_dev, ABS_X, touch_data.sub.curPos);
        //input_sync(cs_input_dev);
    }else{
        LOG_INFO("get curPos err\n");
    }
}
...
static int cs_proc_mode_ctl_show(struct seq_file *m,void *v)
{
	seq_printf(m,"%d\n", cs_mode_result);
    return 0;

}
static int cs_proc_mode_ctl_open(struct inode *inode, struct file *filp)
{
	return single_open(filp,cs_proc_mode_ctl_show,NULL);
}
static ssize_t cs_proc_mode_ctl_write(struct file *file, const char __user *buf,
                                            size_t count, loff_t *offset)
{
    char *kbuf = NULL;
    ...
    if(sscanf(kbuf, "%u", &cs_mode_status) != 1)
    {
	    printk("[cs_proc_mode_ctl_store]: Invalid values\n");
	    return -EINVAL;
    }
	switch(cs_mode_status)
	{
		case 4:
		config_FT_run_mode(SLEEP_MODE);
		break;
		
		case 3:
		config_FT_run_mode(CAMERA_MODE);
		break;
		
		case 2:
		config_FT_run_mode(PROMPT_MODE);
		break;
		
		case 1:
		config_FT_run_mode(DAILY_MODE);
		break;
		
		default:
		break;		
	}
	printk("[cs_proc_mode_ctl_store]: Invalid values = %d\n", cs_mode_status);
exit_kfree:
    kfree(kbuf);
exit:
	return count;	
...
#endif
/*add by driver for gpio start*/
   g_cs_press.vcamvdd_gpio = of_get_named_gpio((pdev->dev).of_node, "vcamvdd-gpio", 0);
   if(!gpio_is_valid(g_cs_press.vcamvdd_gpio))
    {
        dev_err(&pdev->dev, "cs_press request_rst fail");
    } else {
        ret = gpio_request(g_cs_press.vcamvdd_gpio, "vcamvdd-gpio");
            if(ret)
            {
			...
			{
			ret = gpio_direction_output(g_cs_press.vcamvdd_gpio,1);
			}
            msleep(50);
        //gpio_set_value(g_cs_press.vcamvdd_gpio, 0);
    }
	/*
   g_cs_press.iovdd_gpio = of_get_named_gpio((pdev->dev).of_node, "iovdd-gpio", 0);
   if(!gpio_is_valid(g_cs_press.iovdd_gpio))
    {
        dev_err(&pdev->dev, "cs_press request_rst fail");
    } else {
        ret = gpio_request(g_cs_press.iovdd_gpio, "iovdd-gpio");
            if(ret)
            {
                dev_err(&pdev->dev, "cs_press request rst fail !,ret=%d.\n", ret);
            }
			...
			{
			ret = gpio_direction_output(g_cs_press.iovdd_gpio,1);	
			}
            msleep(50);
        //gpio_set_value(g_cs_press.iovdd_gpio, 0);
    }*/
/*add by driver for gpio start*/
...
static const struct file_operations proc_fops[PROC_FOPS_NUM] = {
    ...
    FOPS_ARRAY(cs_proc_mode_ctl_open, cs_proc_mode_ctl_write),
};
/**
  * @brief  cs_sys_create
  * @param
...
static int __maybe_unused cs_suspend(struct device *dev)
{
    enable_irq_wake(cs_press_irq_num);
    LOG_DEBUG("cs_suspend\n");
	return 0;
}

static int __maybe_unused cs_resume(struct device *dev)
{
 ...
 */
static const struct dev_pm_ops cs_press_pm = {
