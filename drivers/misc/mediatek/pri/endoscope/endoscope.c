...
struct endoscope_device{
	...
	struct pinctrl_state *eoc_extra_5v_high;
	struct pinctrl_state *eoc_extra_5v_low;
	
	struct pinctrl_state *endoscope_irq_high;
	struct pinctrl_state *endoscope_ppt_key_high;
	struct pinctrl_state *endoscope_irq_low;
	struct pinctrl_state *endoscope_ppt_key_low;
...
extern void mtk_usb_extcon_set_role_extern(int role);
...
static enum alarmtimer_restart endoscope_alarm_timer_func(struct alarm *alarm, ktime_t now)
{
	...
	if(dev->eoc_irq_sta)
	{
		...
		pinctrl_select_state(dev->eoc_pinctrl, dev->eoc_extra_5v_low);
		gpio_direction_output(dev->eoc_5v_gpio,0);
		mtk_usb_extcon_set_role_extern(0);
	}
	else
	{
		pr_err("++\n");
		pinctrl_select_state(dev->eoc_pinctrl, dev->eoc_switch_high);
		pinctrl_select_state(dev->eoc_pinctrl, dev->eoc_extra_5v_high);
		gpio_direction_output(dev->eoc_5v_gpio,1);
		mtk_usb_extcon_set_role_extern(1);
	}
...
static int endoscope_get_dts_fun(struct endoscope_device* dev)
{
	...
	*/
	...
	dev->eoc_extra_5v_high = pinctrl_lookup_state(dev->eoc_pinctrl, "endoscope_extra_5v_high");
	if (IS_ERR(dev->eoc_extra_5v_high)) {
		ret = PTR_ERR(dev->eoc_extra_5v_high);
		EOC_DEBUG("[endoscope dev]    %s : init err, eoc_extra_5v_high\n", __func__);
	}

	dev->eoc_extra_5v_low = pinctrl_lookup_state(dev->eoc_pinctrl, "endoscope_extra_5v_low");
	if (IS_ERR(dev->eoc_extra_5v_low)) {
		ret = PTR_ERR(dev->eoc_extra_5v_low);
		EOC_DEBUG("[endoscope dev]    %s : init err, eoc_extra_5v_low\n", __func__);
	}
	#if IS_ENABLED(CONFIG_PRI_ENDOSCOPE_LED_TSET)
	dev->endoscope_irq_high = pinctrl_lookup_state(dev->eoc_pinctrl, "endoscope_irq_high");
	if (IS_ERR(dev->endoscope_irq_high)) {
		ret = PTR_ERR(dev->endoscope_irq_high);
		EOC_DEBUG("[endoscope dev]    %s : init err, endoscope_irq_high\n", __func__);
	}
	
	dev->endoscope_ppt_key_high = pinctrl_lookup_state(dev->eoc_pinctrl, "endoscope_ppt_key_high");
	if (IS_ERR(dev->endoscope_ppt_key_high)) {
		ret = PTR_ERR(dev->endoscope_ppt_key_high);
		EOC_DEBUG("[endoscope dev]    %s : init err, endoscope_ppt_key_high\n", __func__);
	}
	dev->endoscope_irq_low = pinctrl_lookup_state(dev->eoc_pinctrl, "endoscope_irq_low");
	if (IS_ERR(dev->endoscope_irq_low)) {
		ret = PTR_ERR(dev->endoscope_irq_low);
		EOC_DEBUG("[endoscope dev]    %s : init err, endoscope_irq_low\n", __func__);
	}
	
	dev->endoscope_ppt_key_low = pinctrl_lookup_state(dev->eoc_pinctrl, "endoscope_ppt_key_low");
	if (IS_ERR(dev->endoscope_ppt_key_low)) {
		ret = PTR_ERR(dev->endoscope_ppt_key_low);
		EOC_DEBUG("[endoscope dev]    %s : init err, endoscope_ppt_key_low\n", __func__);
	}
	#endif
	return 0;
}
...
static int endoscope_irq_init(struct endoscope_device* dev)
{
	...
	dev->eoc_5v_gpio = of_get_named_gpio(pdev->dev.of_node, "extra-5v-gpio", 0);
	if (dev->eoc_5v_gpio < 0) {
		EOC_DEBUG("[endoscope dev]    %s: no 5v gpio provided.\n", __func__);
		return -1;
	} else {
		EOC_DEBUG("[endoscope dev]    %s: 5v gpio provided ok.sy8801_dev->irq_gpio = %d\n", __func__, dev->eoc_irq_gpio);
	}
	
	if (gpio_is_valid(dev->eoc_5v_gpio)) {
		//ret = devm_gpio_request_one(&pdev->dev,dev->eoc_5v_gpio,GPIOF_DIR_OUT, "endoscope_5v");
		ret = devm_gpio_request(&pdev->dev, dev->eoc_5v_gpio, "endoscope_5v");
		if (ret < 0) {
			EOC_DEBUG("[endoscope dev] %s: 5v gpio request failed\n", __func__);
			return -1;
		}
		else{
			EOC_DEBUG("[endoscope dev] %s: 5v gpio request ok\n", __func__);
			gpio_direction_output(dev->eoc_5v_gpio,0);
		}
	}
...
static int endoscope_probe(struct platform_device *pdev)
{
	...
	pinctrl_select_state(eoc_dev->eoc_pinctrl, eoc_dev->eoc_extra_5v_high);
	pinctrl_select_state(eoc_dev->eoc_pinctrl, eoc_dev->endoscope_irq_high);
	pinctrl_select_state(eoc_dev->eoc_pinctrl, eoc_dev->endoscope_ppt_key_high);
	return 0;
	#endif
