                            ...
                            size_t count, loff_t *ppos)
{
    ...
    if(gpio_get_value(pdata->irq) == 0) {
        dev_err(info->dev, "[NFC] irq-gpio state is low when reading, read_irq is %d, continued_count %d!\n",info->i2c_info.read_irq, continued_irq_low_count);
        if(info->i2c_info.read_irq % SEC_NFC_READ_TIMES == 0){
            ...
            if(continued_irq_low_count >= 3){
                ...
                dev_dbg(info->dev, "[NFC] reset read_irq since irq was low for 3 times!\n");
            }
            ret = -EAGAIN;
            mutex_unlock(&info->i2c_info.read_mutex);
            goto out;
    ...
    if (info->i2c_info.read_irq == SEC_NFC_NONE){
        if(gpio_get_value(pdata->irq) > 0) {
            dev_err(info->dev, "[NFC] irq-gpio state is high but read_irq is NONE, trigger interrupt!\n");
            info->i2c_info.read_irq += SEC_NFC_READ_TIMES;
            wake_up_interruptible(&info->i2c_info.read_wait);
        }
    }
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    memset(&current_nfc_info, 0, sizeof(current_nfc_info));
    strcpy(current_nfc_info.chip, nfc_hwinfo[0] ? nfc_hwinfo[0] : "S3NRN4VX");
    strcpy(current_nfc_info.vendor, nfc_hwinfo[1] ? nfc_hwinfo[1] : "Samsung");
    strcpy(current_nfc_info.more, nfc_hwinfo[2] ? nfc_hwinfo[2] : "nfc");
    sprintf(current_nfc_info.id, "0x%04x", info->i2c_info.i2c_dev->addr);
#endif
...
#endif
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    memset(nfc_hwinfo, 0, sizeof(nfc_hwinfo));
    of_property_read_string_array(np, "sec-nfc,hwinfo", nfc_hwinfo, 3);
    pr_info("%s: chip : %s, vendor : %s, more:%s\n",
            __func__,
            nfc_hwinfo[0] ? nfc_hwinfo[0] : "",
            nfc_hwinfo[1] ? nfc_hwinfo[1] : "",
            nfc_hwinfo[2] ? nfc_hwinfo[2] : "");
#endif
