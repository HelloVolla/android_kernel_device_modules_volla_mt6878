...
static int aw36518_enable_ch1(void)
{
	...
	printk("aw36518_is_torch(aw36518_level_ch1)==%d\n",aw36518_is_torch(aw36518_level_ch1));
	
	reg = AW36518_REG_ENABLE;
	if (!aw36518_is_torch(aw36518_level_ch1)) {
		/* torch mode */
		printk(" enter torch mode set.\n");
		aw36518_reg_enable |= AW36518_ENABLE_LED1_TORCH;
	} else {
		/* flash mode */
		printk("enter flash mode set.\n");
	...
	aw36518_i2c_write(aw36518_i2c_client, AW36518_REG_ENABLE, 0x0b);
	
	return aw36518_i2c_write(aw36518_i2c_client, AW36518_REG_ENABLE, 0x0b);
}
...
//drv add by lipengpeng 20240514 start 
static void aw36518_i2c_remove(struct i2c_client *client)
//drv add by lipengpeng 20240514 end  
{
...
static ssize_t device_irtouch_value_show(struct device *dev,
                ...
                char *buf)
{
	printk("aw36518 cur_irtouch_status=%d\n", cur_irtouch_status);
                ...
                const char *buf, size_t size)
{
	...
	printk("aw36518 ON/OFF value = %d:\n ", cur_irtouch_status);
	if(sscanf(buf, "%u", &cur_irtouch_status) != 1)
	{
		printk("aw36518Invalid values\n");
		return -EINVAL;
	}
	...
	if(1==cur_irtouch_status)
	{
		...
		//aw36518_set_level(AW36518_CHANNEL_CH1, 1);
	  //printk("aw36518_set_level[AW36518_CHANNEL_CH1]=%d\n",aw36518_timeout_ms[AW36518_CHANNEL_CH1]);
		//if (aw36518_timeout_ms[AW36518_CHANNEL_CH1]) {
		//	printk("aw36518_timeout_ms[AW36518_CHANNEL_CH1]=%d\n",aw36518_timeout_ms[AW36518_CHANNEL_CH1]);
		//	ktime =ktime_set(aw36518_timeout_ms[AW36518_CHANNEL_CH1] / 1000,
		//	(aw36518_timeout_ms[AW36518_CHANNEL_CH1] % 1000) * 1000000);
		//	aw36518_timer_start(AW36518_CHANNEL_CH1, ktime);
			
		//   }
		   aw36518_enable(AW36518_CHANNEL_CH1);
		   aw36518_i2c_write(aw36518_i2c_client, 0x01, 0x0b);
		
	}else{
			aw36518_disable(AW36518_CHANNEL_CH1);
			aw36518_timer_cancel(AW36518_CHANNEL_CH1);
			aw36518_i2c_write(aw36518_i2c_client, 0x01, 0x00);
 ...
 *****************************************************************************/
 struct class *flash_irtouch_class; //prize add by lipengpeng 20220610 start 
 //struct class *flash_touch_class; //prize add by lipengpeng 20220610 start 
 
static int aw36518_probe(struct platform_device *dev)
{
	
	struct device *flash_irtouch_dev;
	//struct device *flash_touch_dev;

	
	printk(" aw36518 Probe start.\n");
//prize add by lipengpeng 20220610 start 		
