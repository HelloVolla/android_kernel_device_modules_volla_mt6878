...
// "../drivers/gpu/drm/mediatek/mediatek_v2/mtk_panel_ext.h"
...
static struct pinctrl *pri_pinctrl;
static struct pinctrl_state *pri_pins_spi_mode;
static int ff_init_pins(ff_context_t *ff_ctx)
{
    int ret = 0;
	...
	pri_pinctrl = devm_pinctrl_get(&ff_ctx->pdev->dev);
	
	pri_pins_spi_mode = pinctrl_lookup_state(pri_pinctrl, "fp_spi_mode");
    if (IS_ERR_OR_NULL(pri_pins_spi_mode)) {
        ret = PTR_ERR(pri_pins_spi_mode);
        FF_LOGE("fp_spi_mode not found,ret=%d", ret);
        devm_pinctrl_put(pri_pinctrl);
    }else{
	   ret = pinctrl_select_state(pri_pinctrl, pri_pins_spi_mode);
       if (ret < 0) {
			FF_LOGE("pri_pins_spi_mode gpio failed");
	   }else{
		    FF_LOGE("pri_pins_spi_mode gpio driver set ok");
	   }
	}
