...
//prize add by tangcong for ata node 20241017 start
#if IS_ENABLED(CONFIG_PRI_FINGER_ATA_NODE)
extern void pri_ftm_node_exist(int exist);
#endif
//prize add by tangcong for ata node 20241017 end
/*****************************************************************************
...
static int ff_chip_probe_id(void)
{
    ...
    for (read_id_retries = 0; read_id_retries < 3; read_id_retries++) {
		...
		FF_LOGI("tangcong set the state 1");
        #endif
