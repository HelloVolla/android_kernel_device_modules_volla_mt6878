...
*****************************************************************************/
/* DRV added by liaojie, hardware_info, start 
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
#include "../../../../misc/mediatek/hardware_info/hardware_info.h"
extern struct hardware_info current_fingerprint_info;
#endif
 DRV added by liaojie, hardware_info, end */
...
static int ff_set_spiclk(ff_context_t *ff_ctx, bool enable)
{
	...
	if(enable){
		FF_LOGI("'%s' mt_spi_enable_master_clk  enter.", __func__);
		mt_spi_enable_master_clk(ff_ctx->spi);
	}
	else{
		FF_LOGI("'%s' mt_spi_disable_master_clk enter.", __func__);
...
// #ifdef CONFIG_FINGERPRINT_FOCALTECH_SPICLK_SUPPORT
    // if (ff_ctx && ff_ctx->b_spiclk && ff_ctx->spi) {
        // FF_LOGI("set spi clock:%d->%d", ff_ctx->b_spiclk_enabled, enable);
...
int ff_init_driver(void)
{
    ...
    ret = ff_set_spiclk(ff_ctx, 1);
    if (ret < 0) {
        FF_LOGD("enable spi clock fails");
        goto err_enable_spiclk;
    }
...
static int ff_init_pins(ff_context_t *ff_ctx)
{
        ...
        ff_ctx->pins_spi_mode = pinctrl_lookup_state(ff_ctx->pinctrl, "fp_spi_mode");
        if (IS_ERR_OR_NULL(ff_ctx->pins_spi_mode)) {
            ret = PTR_ERR(ff_ctx->pins_spi_mode);
            FF_LOGE("fp_spi_mode not found,ret=%d", ret);
            devm_pinctrl_put(ff_ctx->pinctrl);
            return ret;
        }
        ...
        ret = pinctrl_select_state(ff_ctx->pinctrl, ff_ctx->pins_spi_mode);
        if (ret < 0) {
            FF_LOGE("pinctrl_select_state:pins_spi_mode fails");
            devm_pinctrl_put(ff_ctx->pinctrl);
            ff_ctx->pins_reset_low = NULL;
            ff_ctx->pins_reset_high = NULL;
            ff_ctx->pins_irq_as_int = NULL;
            ff_ctx->pins_power_low = NULL;
            ff_ctx->pins_power_high = NULL;
            ff_ctx->pins_spi_mode = NULL;
            return ret;
        }else
			FF_LOGI("get pins_spi_mode pinctrl success");
                                 ...
                                 unsigned long event, void *data)
{
    ...
    int *blank = NULL;
	ff_context_t *ts_data = container_of(self, ff_context_t, fb_notif);

    if (!evdata) {
        FF_LOGE("evdata is null");
        return 0;
    }
    ...
    blank = evdata->data;
    FF_LOGI("DRM event:%lu,blank:%d", event, *blank);
    switch (*blank) {
    case DRM_PANEL_BLANK_UNBLANK:
        if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
            FF_LOGI("resume: event = %lu, not care\n", event);
        } else if (DRM_PANEL_EVENT_BLANK == event) {
            //queue_work(g_ff_ctx->ts_workqueue, &g_ff_ctx->event_work);
            ff_suspend_resume(ts_data, false);
        }
        break;
    case DRM_PANEL_BLANK_POWERDOWN:
        if (DRM_PANEL_EARLY_EVENT_BLANK == event) {
            //cancel_work_sync(&g_ff_ctx->event_work);
            //fts_ts_suspend(ts_data->fp_dev);
			ff_suspend_resume(ts_data, true);
        } else if (DRM_PANEL_EVENT_BLANK == event) {
            FF_LOGI("suspend: event = %lu, not care\n", event);
        }
        break;
    default:
        FF_LOGI("DRM BLANK(%d) do not need process\n", *blank);
        break;
    }
...
//#if 0
...
#endif //CONFIG_DRM
        //FF_LOGI("notifier,event:%lu,blank:%d", event, blank_value);
        FF_LOGI("notifier,event:%lu", event);
        if (event_enum[1] == event) {
            ...
            //FF_LOGD("notifier,event:%lu,blank:%d, not care", event, blank_value);
			FF_LOGD("notifier,event:%lu, not care", event);
        }
    } else {
        FF_LOGE("ts_data/v is null");
        return -EINVAL;
...
/* DRV added by liaojie, hardware_info, start 
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    sprintf(current_fingerprint_info.chip, "%s", "FT9391SC");
    sprintf(current_fingerprint_info.id,"0x9391");
    sprintf(current_fingerprint_info.vendor, "%s", "FocalTech");
    sprintf(current_fingerprint_info.more, "%s", "Fingerprint");
#endif
 DRV added by liaojie, hardware_info, end */
