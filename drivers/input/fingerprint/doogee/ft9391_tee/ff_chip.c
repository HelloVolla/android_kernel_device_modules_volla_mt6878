...
static int ff_sfr_write(uint8_t addr, uint8_t value)
{
    ...
    if (ret < 0) {
        FF_LOGD("ff_sfr_write fail,addr=0x%x,ret=%d", addr, ret);
    }
...
static int ff_sfr_read(uint8_t addr, uint8_t *value)
{
    ...
    ret = ff_spi_transfer(txbuf, rxbuf, txlen);
    if (ret < 0) {
        FF_LOGD("ff_sfr_read fail,addr=0x%x,ret=%d", addr, ret);
    } else {
        *value = rxbuf[txlen - 1];
    }
    FF_LOGD("'%s' leave.", __func__);
...
static int ff_sram_write(uint16_t addr, uint16_t value)
{
    ...
    ret = ff_spi_transfer(txbuf, rxbuf, txlen);
    if (ret < 0) {
        FF_LOGD("ff_sram_write fail,addr=0x%x,ret=%d", addr, ret);
    }
    FF_LOGD("'%s' leave.", __func__);
    return ret;
}
...
static int ff_sram_read(uint16_t addr, uint16_t *value)
{
    ...
    for (i = 0; i < SPI_RETRY_NUMBER; i++) {
        ret = ff_spi_transfer(txbuf, rxbuf, txlen);
        if (ret < 0) {
            FF_LOGD("ff_sram_read fail,addr=0x%x,ret=%d", addr, ret);
        } else {
            if (FF_SRAM_CRC_EN) {
                uint16_t crc16_calc = ff_crc_calc(&rxbuf[dp], 2);
                uint16_t crc16_read = (rxbuf[dp + 2] << 8) + rxbuf[dp + 3];
                if ((crc16_read) && (crc16_read != crc16_calc)) {
                    FF_LOGD("crc mismatch at addr:0x%x, readout:0x%x != 0x%x", addr, crc16_read, crc16_calc);
                    ret = -EIO;
                    continue;
                }
            }
...
static int ff_chip_probe_id(void)
{
    ...
    for (read_id_retries = 0; read_id_retries < 3; read_id_retries++) {
        ...
        for (i = 0; i < 5; i++) {
            ...
            if (0x7F != val) {
                FF_LOGD("set io voltage abnormal,read:%d,retry:%d", val, i);
                continue;
            }
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (0x01 != val) {
                FF_LOGD("set spi mode abnormal,read:%d,retry:%d", val, i);
                continue;
            }
        ...
        FF_LOGD("read chip id:0x%x", chip_id);
        if ((chip_id & 0xFF00) == 0x9300) {
            FF_LOGD("probe id(0x%x) pass", chip_id);
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
            sprintf(current_fingerprint_info.chip, "FT9399SC");
...
/*prize added by durunshen,ATA test,start*/
            ...
            if(focaltech_kobj == NULL) {
                FF_LOGD("focaltech_kobj create_and_add failed\n");
                ret = -ENOMEM;
                return ret;
            }
...
/*prize added by durunshen,ATA test,start*/
            ...
            FF_LOGD("probe id fail,read:0x%x,retry=%d", chip_id, read_id_retries);
            ret = fts_power_sequence(0);
            if (ret < 0) {
                FF_LOGD("power off fail");
                break;
            }
        }
    }
