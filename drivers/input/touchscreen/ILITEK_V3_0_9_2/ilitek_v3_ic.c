...
/* DRV added by chenjiaxi, hardware_info, start */
...
/* DRV added by chenjiaxi, hardware_info, start */
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.chip,"IC:ILI7807s,FW_VER:%d.%d.%d.%d",buf[1],buf[2],buf[3],buf[4]);
	strcpy(current_tp_info.vendor,"tongxinda");
	sprintf(current_tp_info.id,"0x%06x", ilits->chip->pid >> 8);
	strcpy(current_tp_info.more,"1080*2460");
#endif	
/* DRV added by chenjiaxi, hardware_info, end */
