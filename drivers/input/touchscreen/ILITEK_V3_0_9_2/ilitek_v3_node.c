...
#if defined(CONFIG_TOUCHSCREEN_ILITEK_GESTURE_MODE)
bool ilitek_is_gesture_wakeup_enabled(void)
{
    ...
    return ilits->gesture_enable;
}
EXPORT_SYMBOL_GPL(ilitek_is_gesture_wakeup_enabled);

static ssize_t ilitek_gesture_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    //struct fts_ts_data *ts_data = dev_get_drvdata(dev);

    mutex_lock(&ilits->input->mutex);
    count = snprintf(buf, PAGE_SIZE, "Gesture Mode:%s\n",
                      ilits->gesture_enable ? "On" : "Off");
    mutex_unlock(&ilits->input->mutex);

    return count;
}

static ssize_t ilitek_gesture_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&ilits->input->mutex);
    if (buf[0] == '1') {
        ILI_INFO("enable gesture");
		ilits->gesture = true;
		ilits->gesture_enable = true;
		ILI_INFO("gesture = %d\n", ilits->gesture);
    } else if (buf[0] == '0') {
        ILI_INFO("disable gesture");
		ilits->gesture = false;
		ilits->gesture_enable = false;
		ILI_INFO("gesture = %d\n", ilits->gesture);
    }
    mutex_unlock(&ilits->input->mutex);
...
#if defined(CONFIG_TOUCHSCREEN_ILITEK_GLOVE_MODE)
static ssize_t ilitek_glove_mode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
...
static ssize_t ilitek_glove_mode_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    //int ret = 0;
    ...
    mutex_lock(&ilits->input->mutex);
    if (buf[0] == '1') {
        ILI_INFO("enable glove mode");
		ili_ic_func_ctrl_export("glove", 0x01);
		ilits->glove_enable = true;
		ILI_INFO("glove = %d\n", ilits->glove_enable);
    } else if (buf[0] == '0') {
        ILI_INFO("disable glove mode");
		ili_ic_func_ctrl_export("glove", 0x00);
		ilits->glove_enable = false;
		ILI_INFO("glove = %d\n", ilits->glove_enable);
...
#if defined(CONFIG_TOUCHSCREEN_ILITEK_GESTURE_MODE)
/* sysfs gesture node
 *   read example: cat  ilitek_gesture_mode       ---read gesture mode
 *   write example:echo 1 > ilitek_gesture_mode   --- write gesture mode to 1
 *
 */
static DEVICE_ATTR(gesture, S_IRUGO | S_IWUSR, ilitek_gesture_show,
                   ilitek_gesture_store);
#endif

#if defined(CONFIG_TOUCHSCREEN_ILITEK_GLOVE_MODE)
/* sysfs glove node
 * read example: cat ilitek_glove_mode        ---read  glove mode
 * write example:echo 1 > ilitek_glove_mode   ---write glove mode to 01
 */
static DEVICE_ATTR(state, S_IRUGO | S_IWUSR,
                   ilitek_glove_mode_show, ilitek_glove_mode_store);
#endif


#if defined(CONFIG_TOUCHSCREEN_ILITEK_GESTURE_MODE)
int ilitek_gesture_sysfs_add(struct platform_device *pdev)
{
	int err = 0;
    ILI_INFO("Add device attr groups, ilitek_gesture_sysfs_add\n");
  
	err = device_create_file(&pdev->dev, &dev_attr_gesture);
	if (err) {
        ILI_ERR("sys file creation failed\n");
        return -ENODEV;
	}
	return 0;
}
...
struct ilitek_ts_data *g_ilits_data = NULL;

static int gesture_probe(struct platform_device *pdev)
{
	platform_set_drvdata(pdev, g_ilits_data);
	ilitek_gesture_sysfs_add(pdev);
...
void ili_node_init(void)
{
	...
	err = sysfs_create_link(drv_glove, &ilits->spi->dev.kobj, "common_node");
	if (err) {
		ILI_ERR("prize ilitek sysfs_create_link fail\n");
		return;
	}
	if (sysfs_create_file(&ilits->spi->dev.kobj, &dev_attr_state.attr)) {
		return;
	}
#endif
