...
//drv added by chenjiaxi, hardware_info, begin
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
			sprintf(current_sub_tp_info.chip, "%s,hknd", hyn_6xx_fw[i].chip_name);
#endif
//drv added by chenjiaxi, hardware_info, end
            break;
        }
...
//drv added by chenjiaxi, hardware_info, begin
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
    sprintf(current_sub_tp_info.id,"fw_ver:0x%x", ic->fw_ver);
    sprintf(current_sub_tp_info.more,"touchpanel");
    sprintf(current_sub_tp_info.vendor,"hynitron");
#endif
//drv added by chenjiaxi, hardware_info, end
    return 0;
}
