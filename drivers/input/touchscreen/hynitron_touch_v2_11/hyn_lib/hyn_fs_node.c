...
/* drv add by chenjiaxi, support glove mode, begin */
static ssize_t drv_glove_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int count = 0;
	count = snprintf(buf, PAGE_SIZE, "Sub Glove Mode:%s\n", hyn_fs_data->glove_is_enable ? "On" : "Off");
	return count;
}
...
static ssize_t drv_glove_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	const struct hyn_ts_fuc* hyn_fun = hyn_fs_data->hyn_fuc_used;
	if (buf[0] == '1') {
		hyn_fs_data->glove_is_enable = 1;
		hyn_fun->tp_set_workmode(GLOVE_ENTER, 1);
		pr_err("enable glove mode, glove_mode_enable = %d\n", hyn_fs_data->glove_is_enable);
	} else if (buf[0] == '0') {
		hyn_fs_data->glove_is_enable = 0;
		hyn_fun->tp_set_workmode(GLOVE_EXIT, 1);
...
struct kobject *prize1_obj = NULL;
int drv_glove_init(void)
{
	struct kobject *smartcover_obj = NULL;
	int err;

	prize1_obj = kobject_create_and_add("prize1", kernel_kobj);
	if (IS_ERR(prize1_obj)) {
		return -1;
	}

	smartcover_obj = kobject_create_and_add("smartcover", prize1_obj);
	if (IS_ERR(smartcover_obj)) {
		return -1;
	}

	err = sysfs_create_link(smartcover_obj, &hyn_fs_data->dev->kobj, "common_node");
	if (err) {
		pr_err("sysfs_create_link fail\n");
		return -1;
	}

	err = sysfs_create_file(&hyn_fs_data->dev->kobj, &dev_attr_state.attr);
	if (err) {
...
/* drv add by chenjiaxi, support glove mode, end */
...
int hyn_create_sysfs(struct hyn_ts_data *ts_data)
{
	...
	ret = sysfs_create_file(&hyn_fs_data->dev->kobj, &dev_attr_hyntpfwver.attr);
	if (ret) {
		HYN_ERROR("sysfs_create_file fail\n");
		return -1;
	}

	ret = sysfs_create_file(&hyn_fs_data->dev->kobj, &dev_attr_hyntpdbg.attr);
	if (ret) {
		HYN_ERROR("sysfs_create_file fail\n");
		return -1;
	}

	ret = sysfs_create_file(&hyn_fs_data->dev->kobj, &dev_attr_hynselftest.attr);
	if (ret) {
		HYN_ERROR("sysfs_create_file fail\n");
		return -1;
	}

	ret = sysfs_create_file(&hyn_fs_data->dev->kobj, &dev_attr_hyndumpfw.attr);
	if (ret) {
		HYN_ERROR("sysfs_create_file fail\n");
		return -1;
	}

	ret = sysfs_create_file(&hyn_fs_data->dev->kobj, &dev_attr_hynswitchmode.attr);
	if (ret) {
		HYN_ERROR("sysfs_create_file fail\n");
		return -1;
	}
...
void hyn_release_sysfs(struct hyn_ts_data *ts_data)
{
	...
	if(!IS_ERR_OR_NULL(ts_data->sys_node)){
		sysfs_remove_file(&hyn_fs_data->dev->kobj, &dev_attr_hynswitchmode.attr);
		sysfs_remove_file(&hyn_fs_data->dev->kobj, &dev_attr_hyndumpfw.attr);
		sysfs_remove_file(&hyn_fs_data->dev->kobj, &dev_attr_hynselftest.attr);
		sysfs_remove_file(&hyn_fs_data->dev->kobj, &dev_attr_hyntpdbg.attr);
		sysfs_remove_file(&hyn_fs_data->dev->kobj, &dev_attr_hyntpfwver.attr);
	}
}
