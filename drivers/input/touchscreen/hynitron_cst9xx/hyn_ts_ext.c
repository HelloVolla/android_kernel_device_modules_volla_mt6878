...
int copy_for_updata(struct hyn_ts_data *ts_data,u8 *buf,u32 offset,u16 len)
{
	...
	if(strlen(ts_data->fw_file_name) == 0)
		return ret;
	//fp = filp_open("/sdcard/app.bin",O_RDONLY, 0);//
	...
	set_fs(old_fs);
#else
	ret = kernel_read(fp,pdata, len,&pos);
#endif
	filp_close(fp, NULL);
...
static int hyn_get_threshold(char *filename,char *match_string,s16 *pstore, u16 len)
{
	...
	set_fs(old_fs);
#else
	ret = kernel_read(fp,buf,fsize,&pos);
#endif
...
int factory_multitest(struct hyn_ts_data *ts_data ,char *cfg_path, u8 *data,s16 *test_th,u8 test_item)
{
    ...
    s16 *raw_h,*raw_l,*raw_s,*scap_p,tmp,scap_fix;
    raw_h = (s16*)data;
    raw_l = raw_h + mt_len;
    raw_s = raw_l + mt_len;
    scap_p = raw_s + st_len;
...
int fac_test_log_save(char *log_name,struct hyn_ts_data *ts_data,s16 *test_data, int test_ret)
{
#if 0
#if LINUX_VERSION_CODE <= KERNEL_VERSION(4, 14, 0)
	mm_segment_t old_fs;
