...
static const u8 gesture_key[] = {KEY_U,KEY_UP,KEY_DOWN,KEY_LEFT,KEY_RIGHT,
                                  ...
                                  KEY_S,KEY_V, KEY_C,   KEY_Z,   KEY_PASTE,
                                  KEY_CUT};
...
static void hyn_double_click_type_func(unsigned char on)
{
    if(1 == on) {
        HYN_INFO("[GESTURE]enable gesture");
        gesture_data->gesture_is_enable= ENABLE;
...
#if 0
    pri_common_node_register("GESTURE_SUB", hyn_double_click_type_func);
#endif
    /* pri LAX10-192 added by xuejian 20240418 end */
...
void hyn_gesture_report(struct hyn_ts_data *ts_data)
{
    ...
    /* pri LAX10-192 added by xuejian 20240418 begin */
    HYN_INFO("fw gesture idx:%d\n",ts_data->gesture_id);
    if((ts_data->gesture_id < sizeof(gesture_key)) && (ts_data->gesture_is_enable)){
        input_report_key(dev, gesture_key[ts_data->gesture_id], 1);
        input_sync(dev);
        input_report_key(dev, gesture_key[ts_data->gesture_id], 0);
        input_sync(dev);
    } else {
        HYN_INFO("fw gesture err!\n");
    }
    /* pri LAX10-192 added by xuejian 20240418 end */
}
