...
static ssize_t hyn_dbg_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	if(0 == strcmp(str,"fd")){ //updata fw
		...
		hyn_esdcheck_switch(hyn_fs_data,DISABLE);
		ret = hyn_fun->tp_updata_fw(hyn_fs_data->fw_updata_addr,hyn_fs_data->fw_updata_len);
		hyn_esdcheck_switch(hyn_fs_data,ENABLE);
...
static ssize_t hyn_tpfwver_show(struct device *dev,	struct device_attribute *attr,char *buf)
{
	ssize_t num_read = 0;
    HYN_ENTER();
	num_read = snprintf(buf, 128, "fw_version:0x%02X,chip_type:0x%02X,checksum:0x%02X .\n",
...
static ssize_t hyn_selftest_show(struct device *dev,	struct device_attribute *attr,char *buf)
{
    ...
    if(rbuf == NULL){
        HYN_INFO("zalloc GFP_KERNEL memory[%d] failed.\n",max_len);
        return -ENOMEM;
    }
...
static DEVICE_ATTR(hyntpdbg, S_IRUGO | S_IWUSR, hyn_dbg_show,
			hyn_dbg_store);
static DEVICE_ATTR(hynselftest, S_IRUGO | S_IWUSR, hyn_selftest_show,
			hyn_selftest_store);
