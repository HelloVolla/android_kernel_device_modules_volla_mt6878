...
static void hyn_resum(struct device *dev)
{
    ...
    u8 glove_buf[] = {
        0xD0, 0x01, 0x00, 0x00,
        0x01, 0x20, 0x0F, 0x08, 0x02,
        0x02, 0x00, 0x00, 0x88, 0x01,
        0x00, 0x05, 0x05, 0x01, 0x06,
        0x07, 0x03, 0xC8, 0x14, 0x00,
    ...
    /* drv add by chenjiaxi, support glove mode, begin */
    if (hyn_data->glove_state) {
        ret = hyn_write_data(hyn_data, glove_buf, 4, 29);
        ret |= hyn_wr_reg(hyn_data, 0xD0000611, 4, 0, 0);
        mdelay(1);
        ret |= hyn_wr_reg(hyn_data, 0xD0000DAB, 4, 0, 0);
    }
    /* drv add by chenjiaxi, support glove mode, end */
}

/* drv add by chenjiaxi, add drm mediatek v2, begin */
#if defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_FB) || IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
/* drv add by chenjiaxi, add drm mediatek v2, end */
...
extern int drv_mtk_disp_sub_notifier_register(const char *source, struct notifier_block *nb);
extern int drv_mtk_disp_sub_notifier_unregister(struct notifier_block *nb);

static int drv_mtk_disp_sub_notifier_callback(struct notifier_block *self, unsigned long val, void *v)
{
    int *data = (int *)v;

	// HYN_INFO("FB val:%lu, data:%d", val, *data);

	if (val == MTK_DISP_EVENT_BLANK) {
		if (*data == MTK_DISP_BLANK_UNBLANK) {
			hyn_resum(hyn_data->dev);
			return NOTIFY_OK;
		}
	} else if (val == MTK_DISP_EARLY_EVENT_BLANK) {
...
/* drv add by chenjiaxi, add drm mediatek v2, begin */
#elif IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
    ts_data->drv_mtk_disp_sub_notif.notifier_call = drv_mtk_disp_sub_notifier_callback;
    ret = drv_mtk_disp_sub_notifier_register("hyn", &ts_data->drv_mtk_disp_sub_notif);
    if (ret)
        HYN_ERROR("register drv_mtk_disp_sub_notifier_register failed: %d", ret);
/* drv add by chenjiaxi, add drm mediatek v2, end */
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
/* drv add by chenjiaxi, add drm mediatek v2, begin */
#elif IS_ENABLED(CONFIG_DRM_MEDIATEK_V2)
        drv_mtk_disp_sub_notifier_unregister(&ts_data->drv_mtk_disp_sub_notif);
/* drv add by chenjiaxi, add drm mediatek v2, end */
#elif defined(CONFIG_HAS_EARLYSUSPEND)
