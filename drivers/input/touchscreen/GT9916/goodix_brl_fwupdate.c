...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
	...
	if (fwu_ctrl->core_data->bus->ic_type == IC_TYPE_BERLIN_D ||
	    ...
	    fwu_ctrl->core_data->bus->ic_type == IC_TYPE_ATB)
				 ...
				 struct goodix_flash_cmd *flash_cmd)
{
	...
	ts_info(dev, "try send flash cmd:%*ph", (int)sizeof(flash_cmd->buf),
		flash_cmd->buf);
	...
	for (i = 0; i < retry; i++) {
		...
		ts_info(dev, "flash cmd ack error retry %d, ack 0x%x, ret %d", i,
			tmp_cmd.ack, ret);
	}
	...
	ts_info(dev, "flash cmd ack check pass");
	...
	for (i = 0; i < retry; i++) {
		...
		if (!ret && tmp_cmd.ack == FLASH_CMD_ACK_CHK_PASS &&
			...
			ts_info(dev, "flash status check pass");
			return 0;
		}

		ts_info(dev, "flash cmd status not ready, retry %d, ack 0x%x, status 0x%x, ret %d",
	...
	if (ret) {
		ts_info(dev, "reason: bus or paltform error");
		return -EINVAL;
	}
				...
				u8 *pkg, u32 flash_addr, u16 pkg_len)
{
	...
	do {
		...
		if (!ret) {
			ts_info(dev, "success write package to 0x%05X, len %d",
				  ...
				  struct fw_subsys_info *subsys)
{
	...
	while (total_size > 0) {
		data_size = total_size > ISP_MAX_BUFFERSIZE ?
				    ISP_MAX_BUFFERSIZE :
				    total_size;
		ts_info(dev, "Flash firmware to 0x%05X,size:%u bytes",
...
static int goodix_flash_firmware(struct fw_update_ctrl *fw_ctrl)
{
	...
	struct goodix_cfg_bundle *cfgb = &fw_ctrl->core_data->cfg_bundle;
	struct firmware_summary *fw_summary;
	...
	for (i = 1; i < fw_num && retry;) {
		ts_info(dev, "--- Start to flash subsystem[%d] ---", i);
