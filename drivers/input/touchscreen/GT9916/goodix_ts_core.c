...
int g_pdev_id;
extern bool doze_en;//modfiy by wanwen for tp obtain the AOD status 20250925
...
static void goodix_ts_procfs_init(struct goodix_ts_core *cd)
{
	...
	if (dev_id == 0)
		sprintf(proc_node, "goodix_ts_gt9916");
	else
		sprintf(proc_node, "goodix_ts_gt9916.%d", dev_id);
...
static void goodix_ts_procfs_exit(struct goodix_ts_core *cd)
{
	...
	remove_proc_entry("tp_raw", NULL);
	remove_proc_entry("tp_diff", NULL);
			   ...
			   struct goodix_ts_board_data *board_data)
{
	...
	ts_info(dev, "[DT]x:%d, y:%d, w:%d, p:%d sleep_enable:%d pen_enable:%d",
...
#if IS_ENABLED(CONFIG_AW95016)
/**
 * goodix_ts_suspend_gesture_power_on - Turn on power to suspend gesture
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
int gt9916_suspend_gesture_power_on(struct goodix_ts_core *cd)
{
	struct device *dev = cd->bus->dev;
	int ret = 0;

	ts_info(dev, "[swb][GT9916]Device suspend gesture power on");
	if (cd->power_on)
		return 0;

	ret = cd->hw_ops->suspend_gesture_power_on(cd, true);
	if (!ret)
		cd->power_on = 1;
	else
		ts_err(dev, "[swb][GT9916]failed suspend gesture power on, %d", ret);
	return ret;
}

/**
 * goodix_ts_suspend_gesture_power_off - Turn off power to suspend gesture
 * @core_data: pointer to touch core data
 * return: 0 ok, <0 failed
 */
int gt9916_suspend_gesture_power_off(struct goodix_ts_core *cd)
{
	struct device *dev = cd->bus->dev;
	int ret;
	...
	ts_info(dev, "[swb][GT9916]Device suspend gesture power off");
	ret = cd->hw_ops->suspend_gesture_power_on(cd, false);
	if (!ret)
		cd->power_on = 0;
	else
		ts_err(dev, "[swb][GT9916]failed suspend gesture power off, %d", ret);
...
static int goodix_ts_gpio_setup(struct goodix_ts_core *core_data)
{
...
#if !IS_ENABLED(CONFIG_AW95016) 
	ts_info(dev, "GPIO setup,reset-gpio:%d", ts_bdata->reset_gpio);
	/*
	...
	ts_info(dev, "GPIO setup,irq-gpio:%d", ts_bdata->irq_gpio);
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
    ...
    atomic_set(&core_data->suspendding, 1);
	...
	if (core_data->gesture_type) {
		/* enter gesture mode */
		ts_info(dev, "[swb][GT9916]enter gesture mode[0x%x]", core_data->gesture_type);
		hw_ops->gesture(core_data, core_data->gesture_type);
		hw_ops->irq_enable(core_data, true);
    ...
    atomic_set(&core_data->suspendding, 0);
...
static int goodix_ts_resume(struct goodix_ts_core *core_data)
{
    ...
    atomic_set(&core_data->resumming, 1);
	atomic_set(&core_data->suspended, 0);
	hw_ops->irq_enable(core_data, false);

	if (core_data->gesture_type) {
        ts_info(dev, "gesture_type Resume");
		disable_irq_wake(core_data->irq);
		hw_ops->resume(core_data);
	} else {
		/* reset device or power on*/
        //drv-mod by shenwenbin for electric leakage with not poweroff 20250918 start
        if (core_data->board_data->sleep_enable){
            ts_info(dev, "sleep_enable Resume");
            goodix_ts_power_off(core_data);
            usleep_range(5000, 5100);
            ...
            ts_info(dev, "goodix_ts_power_on Resume");
			goodix_ts_power_on(core_data);
        }
        //drv-mod by shenwenbin for electric leakage with not poweroff 20250918 end
	}
    ...
    atomic_set(&core_data->resumming, 0);
...
//modfiy by wanwen for tp obtain the AOD status 20250925 start
	if(doze_en == true){
        return 0;
    }
//modfiy by wanwen for tp obtain the AOD status 20250925 end
                ...
                struct panel_event_notification *notification, void *client_data)
{
    ...
    case DRM_PANEL_EVENT_BLANK:
        if (notification->notif_data.early_trigger)
            ts_info(dev, "resume notification pre commit");
        else {
            cancel_work_sync(&core_data->resume_work);
            goodix_ts_suspend(core_data);
        }
    ...
    case DRM_PANEL_EVENT_UNBLANK:
        if (notification->notif_data.early_trigger)
            ts_info(dev, "suspend notification pre commit");
        else {
            schedule_work(&core_data->resume_work);
        }
        break;
...
int goodix_ts_stage2_init(struct goodix_ts_core *cd)
{
    ...
    if (mtk_disp_sub_notifier_register(GOODIX_CORE_DRIVER_NAME, &cd->pm_notifier))
...
static int goodix_later_init_thread(void *data)
{
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_sub_tp_info.chip,"GT9916,CFG:0x%x,FW:0x%02x%02x%02x%02x",
	...
	strcpy(current_sub_tp_info.id,"9916");
    strcpy(current_sub_tp_info.vendor,"GOODIX");
    sprintf(current_sub_tp_info.more,"%d*%d",1172,2748);
#endif
...
static struct platform_driver goodix_ts_driver = {
	.driver = {
		.name = GOODIX_CORE_DRIVER_NAME,
		.owner = THIS_MODULE,
#if IS_ENABLED(CONFIG_PM) && !IS_ENABLED(CONFIG_DRM_MEDIATEK_V2) && !IS_ENABLED(CONFIG_DRM_PANEL_NOTIFY) && !IS_ENABLED(CONFIG_FB)
		.pm = &dev_pm_ops,
#endif
	},
