...
static bool gtp_double_type_func(unsigned char on)
{
    ...
    if(atomic_read(&gtp_core->suspended))
    {
        if(1 == on){
           ...
           gt9916_suspend_gesture_power_on(gtp_core);
#endif
           if (gtp_core->board_data->sleep_enable){
               hw_ops->resume(gtp_core);
           }
           gtp_core->gesture_type |= GESTURE_DOUBLE_TAP;
           ts_info(dev, "[swb][GT9916]%s gtp_core->suspended = %d;enter DOUBLE-TAP gesture\n", __func__,atomic_read(&gtp_core->suspended));
		   ...
		   enable_irq_wake(gtp_core->irq);  
            ...
            if (gtp_core->board_data->sleep_enable){
                hw_ops->suspend(gtp_core);
            }
#if IS_ENABLED(CONFIG_AW95016)
            gt9916_suspend_gesture_power_off(gtp_core);
#endif
            ts_info(dev, "[swb][GT9916]%s gtp_core->suspended = %d;close DOUBLE-TAP gesture\n", __func__,atomic_read(&gtp_core->suspended)); 
        }  
    }
    else
    {
    ...
    gt9916_gesture_status = !!(gtp_core->gesture_type & GESTURE_DOUBLE_TAP);
    return gt9916_gesture_status;
}
#endif
//drv-mod by shenwenbin for double tap function 20250830 end
...
#if IS_ENABLED(CONFIG_PRI_COMMON_NODE)
	gtp_core = cd;
	pri_common_node_register("GESTURE_SUB", &gtp_double_type_func);
#endif
//drv-mod by shenwenbin for double tap function 20250830 end
