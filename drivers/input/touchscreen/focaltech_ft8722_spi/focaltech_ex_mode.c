...
#if defined(CONFIG_TOUCHSCREEN_FOCALTECH_FT8722_GLOVE_SUPPORT)
/* DRV modified by chenjiaxi, glove mode, start */
static DEVICE_ATTR(state, S_IRUGO | S_IWUSR,
                   fts_glove_mode_show, fts_glove_mode_store);
/* DRV modified by chenjiaxi, glove mode, end */
#endif
...
int fts_ex_mode_init(struct fts_ts_data *ts_data)
{
...
#if defined(CONFIG_TOUCHSCREEN_FOCALTECH_FT8722_GLOVE_SUPPORT)
	/* DRV added by chenjiaxi, glove mode, start */
	...
	err = sysfs_create_link(drv_glove, &ts_data->spi->dev.kobj, "common_node");
	if (err) {
		FTS_ERROR("prize fts sysfs_create_link fail\n");
		return -1;
	}
	if (sysfs_create_file(&ts_data->spi->dev.kobj, &dev_attr_state.attr)) {
		return -1;
	}
	/* DRV added by chenjiaxi, glove mode, end */
#endif
