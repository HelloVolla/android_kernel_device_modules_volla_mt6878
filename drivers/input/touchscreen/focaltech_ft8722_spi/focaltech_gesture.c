...
/* DRV added by wangwei1 20240809, gestuer node, start */
#if defined(CONFIG_PRI_COMMON_NODE)
static struct fts_ts_data *ft8722_data;
static void ft8722_double_type_func(unsigned char on)
{
    if (ft8722_data->suspended) {
        FTS_INFO("In suspend,not operation gesture mode!");
        return;
    }

    mutex_lock(&ft8722_data->input_dev->mutex);
	if(1 == on){
        FTS_INFO("enable gesture");
        ft8722_data->gesture_support = ENABLE;
	}else if(0 == on){
        FTS_INFO("disable gesture");
        ft8722_data->gesture_support = DISABLE;
	}
    mutex_unlock(&ft8722_data->input_dev->mutex);
...
#if defined(CONFIG_PRI_COMMON_NODE)
	ft8722_data = ts_data;
	pri_common_node_register("GESTURE", &ft8722_double_type_func);
#endif
/* DRV modified by wangwei1 20240809, gesture mode, end */
