...
#include "../../../misc/mediatek/pri/aw95016/aw95016.h"
#endif
...
#define GOODIX_IC_INFO_MAX_LEN 256
...
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	...
	if (on) {
        pri_aw95016a_gpio_set(aw95016_ext_gpio_P0_3, 1, 1);//TSP_VDDI
		usleep_range(10000, 10100);
        pri_aw95016a_gpio_set(aw95016_ext_gpio_P0_4, 1, 1);//TSP_AVDD
		usleep_range(15000, 15100);
		pri_aw95016a_gpio_set(aw95016_ext_gpio_P1_2, 1, 1);//RST
...
power_off:
	pri_aw95016a_gpio_set(aw95016_ext_gpio_P1_2, 1, 0);
    usleep_range(15000, 15100);
    pri_aw95016a_gpio_set(aw95016_ext_gpio_P0_4, 1, 0);
	return ret; 
}

//drv-mod by shenwenbin for double tap function 20250903 start
static int brl_suspend_gesture_power_on(struct goodix_ts_core *cd, bool on)
{
	int ret = 0;
...
power_off:
	pri_aw95016a_gpio_set(aw95016_ext_gpio_P1_2, 1, 0); //RST
    usleep_range(15000, 15100);
    pri_aw95016a_gpio_set(aw95016_ext_gpio_P0_4, 1, 0); //TSP_AVDD
    usleep_range(15000, 15100);
    pri_aw95016a_gpio_set(aw95016_ext_gpio_P0_3, 1, 0); //TSP_VDDI
	usleep_range(10000, 10100);
	return ret; 
}
//drv-mod by shenwenbin for double tap function 20250903 end
#else
static int brl_power_on(struct goodix_ts_core *cd, bool on)
{
	struct device *dev = cd->bus->dev;
...
static int brl_reset(struct goodix_ts_core *cd, int delay)
{
	...
	pri_aw95016a_gpio_set(aw95016_ext_gpio_P1_2, 1, 1);
	if (delay < 20)
...
static int brl_irq_enbale(struct goodix_ts_core *cd, bool enable)
{
	...
	ts_info(dev, "warnning: irq deepth inbalance!");
	return 0;
}
...
static int parse_ic_info_v2(struct goodix_ts_core *cd, const u8 *data)
{
	...
	u8 extend_buf[GOODIX_IC_INFO_MAX_LEN] = { 0 };
	u8 exit_doze[] = { 0x00, 0x00, 0x07, 0xDF, 0x00, 0x00, 0x00, 0xE6, 0x00 };
	int i;
	...
	/* parse info v2 version pkg */
	memcpy(ver_pkg, data, sizeof(*ver_pkg));
	data += sizeof(*ver_pkg);

	/* parse info v2 bsp pkg */
	memcpy(bsp_pkg, data, sizeof(*bsp_pkg));
	data += sizeof(*bsp_pkg);

	/* parse info v2 param pkg */
	data += 4;

	param_pkg->active_scan_rate_num = *data++;
	for (i = 0; i < param_pkg->active_scan_rate_num; i++)
		param_pkg->active_scan_rate[i] = le16_to_cpup((__le16 *)(data + i * 2));
	data += param_pkg->active_scan_rate_num * 2;

	param_pkg->mutual_freq_num = *data++;
	for (i = 0; i < param_pkg->mutual_freq_num; i++)
		param_pkg->mutual_freq[i] = le16_to_cpup((__le16 *)(data + i * 2));
	data += param_pkg->mutual_freq_num * 2;

	param_pkg->self_tx_freq_num = *data++;
	for (i = 0; i < param_pkg->self_tx_freq_num; i++)
		param_pkg->self_tx_freq[i] = le16_to_cpup((__le16 *)(data + i * 2));
	data += param_pkg->self_tx_freq_num * 2;

	param_pkg->self_rx_freq_num = *data++;
	for (i = 0; i < param_pkg->self_rx_freq_num; i++)
		param_pkg->self_rx_freq[i] = le16_to_cpup((__le16 *)(data + i * 2));
	data += param_pkg->self_rx_freq_num * 2;

	param_pkg->stylus_freq_num = *data++;
	for (i = 0; i < param_pkg->stylus_freq_num; i++)
		param_pkg->stylus_freq[i] = le16_to_cpup((__le16 *)(data + i * 2));
	data += param_pkg->stylus_freq_num * 2;

	/* parse info v2 addr pkg */
	memcpy(addr_pkg, data, sizeof(*addr_pkg));
	data += sizeof(*addr_pkg);

	/* parse info v2 protocol pkg */
	memcpy(protocol_pkg, data, sizeof(*protocol_pkg));
	data += sizeof(*protocol_pkg);

	ts_info(dev, "extend_buffer_addr:0x%04x len:%d", head->info_extend_buffer_addr, head->info_extend_len);
	/* parse info v2 extend pkg */
	if (head->info_extend_buffer_addr > 0 &&
			head->info_extend_buffer_addr < 0xFFFFFF &&
			head->info_extend_len > 0) {
		cd->hw_ops->write(cd, 0x10174, exit_doze, sizeof(exit_doze));
		msleep(20);
		cd->hw_ops->read(cd, head->info_extend_buffer_addr, extend_buf, head->info_extend_len);
		if (checksum_cmp((const uint8_t *)extend_buf, head->info_extend_len,
				 CHECKSUM_MODE_U8_LE)) {
			ts_info(dev, "extend ic info checksum error!");
			return -EINVAL;
		}
		memcpy(extend_rep_pkg, extend_buf, sizeof(*extend_rep_pkg));
		memcpy(extend_addr_pkg, extend_buf + sizeof(*extend_rep_pkg), sizeof(*extend_addr_pkg));
	}
...
//modify by shenwenbin for ESD TP recovery 20250909 start
extern int mtk_drm_esd_check_status_tp(void);
extern void mtk_drm_esd_set_status_tp(int status);
//modify by shenwenbin for ESD TP recovery 20250909 end
static int brl_esd_check(struct goodix_ts_core *cd)
{
    ...
    //modify by shenwenbin for ESD TP recovery 20250909 start    
    ret = mtk_drm_esd_check_status_tp();
    if (ret) {
        ts_err(dev, "lcm happened recovery");
        mtk_drm_esd_set_status_tp(0);
		return ret;
	}
    //modify by shenwenbin for ESD TP recovery 20250909 end
...
static unsigned int goodix_touch_btn_code[] = { KEY_0, KEY_1, KEY_2, KEY_3,
...
//drv-mod by shenwenbin for double tap function 20250903 start
#if IS_ENABLED(CONFIG_AW95016)
    .suspend_gesture_power_on = brl_suspend_gesture_power_on,
#endif
//drv-mod by shenwenbin for double tap function 20250903 end
	.resume = brl_resume,
	.suspend = brl_suspend,
