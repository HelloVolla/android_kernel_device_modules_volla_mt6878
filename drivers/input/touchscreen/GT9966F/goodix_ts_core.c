...
static void goodix_ts_procfs_init(struct goodix_ts_core *cd)
{
	...
	if (dev_id == 0)
		sprintf(proc_node, "goodix_ts_gt9966f");
	else
		sprintf(proc_node, "goodix_ts_gt9966f.%d", dev_id);
 ...
 * return: 0 ok, <0 failed
 */
int gt9966f_suspend_gesture_power_on(struct goodix_ts_core *cd)
{
	struct device *dev = cd->bus->dev;
	int ret = 0;

	ts_info(dev, "[swb][GT9966F]Device suspend gesture power on");
	if (cd->power_on)
		return 0;
	...
	if (!ret)
		...
		ts_err(dev, "[swb][GT9966F]failed suspend gesture power on, %d", ret);
	return ret;
}
 ...
 * return: 0 ok, <0 failed
 */
int gt9966f_suspend_gesture_power_off(struct goodix_ts_core *cd)
{
	struct device *dev = cd->bus->dev;
	int ret;
	...
	ts_info(dev, "[swb][GT9966F]Device suspend gesture power off");
	ret = cd->hw_ops->suspend_gesture_power_on(cd, false);
	if (!ret)
		cd->power_on = 0;
	else
		ts_err(dev, "[swb][GT9966F]failed suspend gesture power off, %d", ret);
...
static int goodix_ts_suspend(struct goodix_ts_core *core_data)
{
	...
	if (core_data->gesture_type) {
		/* enter gesture mode */
		ts_info(dev, "[swb][GT9966F]enter gesture mode[0x%x]", core_data->gesture_type);
		hw_ops->gesture(core_data, core_data->gesture_type);
		hw_ops->irq_enable(core_data, true);
...
//modfiy by wanwen for obtain the AOD status 20250925 start
    if(doze_en == true){
        return 0;
    }
//modfiy by wanwen for obtain the AOD status 20250925 start
...
static int goodix_later_init_thread(void *data)
{
...
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	sprintf(current_tp_info.chip,"GT9966F,CFG:0x%x,FW:0x%02x%02x%02x%02x",
	...
	strcpy(current_tp_info.id,"9966F");
    strcpy(current_tp_info.vendor,"GOODIX");
    sprintf(current_tp_info.more,"%d*%d",2480,2200);
#endif
