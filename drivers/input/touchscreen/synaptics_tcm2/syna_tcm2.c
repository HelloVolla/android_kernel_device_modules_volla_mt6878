...
/* pri LAX10-1020 modify by ocean 20240621 start */
#define PALM_NEAR 0
#define PALM_FAR  5
#define TOUCH_FOD_DOWN 128
#define TOUCH_FOD_MOVE 131
#define TOUCH_FOD_UP   129
/* pri LAX10-1020 modify by ocean 20240621 end */
...
#define FW_IMAGE_NAME_GENERALDUTY "reflash_firmware_generalduty.img"
#define MATCH_TABLE_SIZE (sizeof(matchTable) / sizeof(matchTable[0]))
static const BootConfigMatch matchTable[] = {
    {{0x48, 0x30, 0x39, 0x35}, FW_IMAGE_NAME_GENERALDUTY},
    {{0x00, 0x00, 0x00, 0x00}, FW_IMAGE_NAME},
};
...
#if IS_ENABLED(CONFIG_CS_NOTIFIER)
static int syna_tcm_usb_notifier_callback(struct notifier_block *nb, unsigned long event, void *data);
#endif
 ...
 * Detect the palm event and notify the sensor
 *  @status  palm event
*/
void update_palm_sensor_value(int status)
{

	return;
//	struct touch_event_report_data touch_event;
//	switch (status)
//	{
//	case PALM_NEAR:
//		touch_event.trigger = ANTIFAKE_TOUCH_NEAR;
//		break;
//	case PALM_FAR:
//		touch_event.trigger = ANTIFAKE_TOUCH_FAR;
//		break;
//	default:
//		touch_event.trigger = ANTIFAKE_TOUCH_UNKNOW;
//		break;
//	}
//	cs_touch_notifier_call_chain(CS_TOUCH_EVENT_DATA, &touch_event);
}
...
static void syna_dev_report_input_events(struct syna_tcm *tcm)
{
...
#ifdef ENABLE_WAKEUP_GESTURE
	LOGI("%d,%d,pwr_state:%d, irq_wake:%d,gesture_id=%d,last_gesture_id=%d\n",tcm->wakeup_click_enabled,
				tcm->finger_fod_enabled,tcm->pwr_state,tcm->irq_wake,touch_data->gesture_id,last_gesture_id);
	if (((tcm->pwr_state == LOW_PWR)||(tcm->pwr_state == PWR_ON)) && tcm->irq_wake) {
		...
		case 1:
			...
			break;
		case TOUCH_FOD_DOWN:     /* 0x80 */
		case TOUCH_FOD_MOVE:     /* 0x83 */
...
#endif
/* pri LAX10-1020 modify by ocean 20240621 start */
			if (tcm->aft_enabled && (tcm->palm_sensor_status == PALM_NEAR)) {
				tcm->palm_sensor_status = PALM_FAR;
				update_palm_sensor_value(PALM_FAR);
				LOGI("update palm sensor value PALM_FAR");
			}
			break;
		case PALM:
			if (tcm->aft_enabled && (tcm->palm_sensor_status == PALM_FAR)) {
				tcm->palm_sensor_status = PALM_NEAR;
				update_palm_sensor_value(PALM_NEAR);
				LOGI("update palm sensor value PALM_NEAR");
			}
			break;
/* pri LAX10-1020 modify by ocean 20240621 end */
		case FINGER:
...
static int syna_dev_check_input_params(struct syna_tcm *tcm)
{
	...
	if (tcm_dev->max_objects > MAX_NUM_OBJECTS) {
		LOGI("Out of max num objects defined, in app_info: %d\n",
			tcm_dev->max_objects);
		return 0;
	}

	LOGI("Input parameters unchanged\n");
...
static void syna_dev_reflash_startup_work(struct work_struct *work)
{
	...
	tcm_dev = tcm->tcm_dev;
	LOGI("syna_dev_reflash_startup_work++\n");
/*pri LAX10-651 modify by ocean 20240615 start*/
	rd_buffer.buf = NULL;
	retval = syna_tcm_read_flash_area(tcm_dev,AREA_BOOT_CONFIG,&rd_buffer, DEFAULT_FLASH_READ_DELAY);

	if (rd_buffer.buf) {
		for (unsigned int i = 0; i < MATCH_TABLE_SIZE && !matched; ++i) {
			if (memcmp(rd_buffer.buf, matchTable[i].code, sizeof(matchTable[i].code)) == 0) {
				retval = request_firmware(&fw_entry, matchTable[i].firmwareName, tcm->pdev->dev.parent);
				if (retval < 0) {
					LOGE("Fail to request %s\n", matchTable[i].firmwareName);
					return;
				}
				tcm->fw_boot_status = i;
				LOGI("request %s\n", matchTable[i].firmwareName);
				matched = true;
			}
		}
		...
		if (!matched) {
			for (unsigned int i = 0; i < rd_buffer.data_length; ++i) {
				LOGE("%02X", rd_buffer.buf[i]);
			}
			retval = request_firmware(&fw_entry,FW_IMAGE_NAME_GENERALDUTY,tcm->pdev->dev.parent);
			if (retval < 0) {
				LOGE("Fail to request %s\n", FW_IMAGE_NAME_GENERALDUTY);
				return;
			}
		}
	} else {
...
/* pri LAX10-192 added by xuejian 20240410 begin*/
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	snprintf (current_tp_info.chip, 25, "S3909_FW:0x%x", tcm_dev->app_info.customer_config_id[15]);
	strcpy(current_tp_info.vendor," synaptics");
	strcpy(current_tp_info.id," 0x00");
	strcpy(current_tp_info.more,"1200*2652");
#endif
/* pri LAX10-192 added by xuejian 20240410 end*/
...
static int syna_dev_resume(struct device *dev)
{
	...
	if (hw_if->ops_hw_reset) {
		...
		retval = syna_tcm_get_event_data(tcm->tcm_dev,
			&status, NULL);
		LOGI(" to complete hw reset retval =%d status=%d\n",retval,status);
...
/*pri LAX10-1016 modify by ocean 20240619 start*/
#ifdef ENABLE_WAKEUP_GESTURE
	if (tcm->finger_fod_enabled && (atomic_read(&tcm->fod_figer_state) == 1)) {
		queue_delayed_work(tcm->resume_queue, &tcm->fod_work,msecs_to_jiffies(500));
	}
#endif
/*pri LAX10-1016 modify by ocean 20240619 end*/
#if IS_ENABLED(CONFIG_CS_NOTIFIER)
	if(tcm->usb_state) {
		retval = syna_tcm_set_dynamic_config(tcm->tcm_dev, 0X07, 1,RESP_IN_ATTN);
		if (retval < 0) {
			LOGE("Fail to set charge type fail\n");
		}
		LOGI("Charging plug!\n");
	}
#endif
/*pri LAX10-1016 modify by ocean 20240705 start*/
	if(tcm->aft_enabled) {
		retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
			DC_PALM_AREA_CHANGE, 1,RESP_IN_ATTN);
		if(retval < 0) {
			LOGE("Fail to set palm fail\n");
		}
	}
/*pri LAX10-1016 modify by ocean 20240705 end*/
...
#ifdef ENABLE_WAKEUP_GESTURE
//drv-Fix the fingerprint unlock under the Changan screen, this time press the power button to wake up sleep resulting in TP no touch
    atomic_set(&tcm->fod_figer_state, 0);
	last_gesture_id = 0;
#endif
...
/*pri LAX10-651 modify by ocean 20240611 start*/
/**
 * cs_notifier_callback()
 *
                               ...
                               unsigned long event, void *data)
{
	...
	LOGI("notifier,event:%lu,blank:%d", event, blank_value);

	switch(event) {
	case CS_PANEL_EARLY_EVENT_BLANK:
		if (blank_value == PANEL_BLANK_UNBLANK) {
			queue_delayed_work(tcm->resume_queue, &tcm->resume_work,msecs_to_jiffies(0));
		}
		break;
	case CS_PANEL_EVENT_BLANK:
		...
		case PANEL_BLANK_DOZE_ENABLE:
			cancel_delayed_work_sync(&tcm->resume_work);
		...
		break;
	default:
		LOGI("notifier,event:%lu,blank:%d, not care", event, blank_value);
		break;
	}
...
/*pri LAX10-1020 modify by ocean 20240628 start*/
static int cs_sensor_notifier_callback(struct notifier_block *nb,
                               unsigned long event, void *data)
{
	int retval;
	struct sensor_dev_state *sensor_state = data;
	int state_value = sensor_state->state;
	struct syna_tcm *tcm = container_of(nb, struct syna_tcm, sensor_notifier);

	LOGI("sensor notifier,event:%lu, state:%d", event, state_value);

	switch(event) {
	case CS_SENSOR_EVENT_STATE:
		tcm->aft_enabled = state_value;
/*pri LAX10-1016 modify by ocean 20240705 start*/
		if (tcm->aft_enabled && atomic_read(&tcm->now_system_status) == 1) {
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
				DC_PALM_AREA_CHANGE, 1,RESP_IN_ATTN);
			if (retval < 0) {
			...
			tcm->palm_sensor_status = PALM_FAR;
		} else if (!tcm->aft_enabled && atomic_read(&tcm->now_system_status) == 1) {
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev,
				DC_PALM_AREA_CHANGE, 0,RESP_IN_ATTN);
			if (retval < 0) {
				LOGE("Failed to palm area change setting, retval=%d\n", retval);
			}
		}
/*pri LAX10-1016 modify by ocean 20240705 end*/
	break;
	default:
		LOGI("notifier,event:%lu,blank:%d, not care", event, state_value);
		break;
	}

	return 0;
}
/*pri LAX10-1020 modify by ocean 20240628 end*/
...
#else
	disp_blank_powerdown = MTK_DISP_BLANK_POWERDOWN;
	disp_early_event_blank = MTK_DISP_EARLY_EVENT_BLANK;
	disp_blank = MTK_DISP_EVENT_BLANK;
	disp_blank_unblank = MTK_DISP_BLANK_UNBLANK;
#endif
...
/*pri LAX10-651 modify by ocean 20240611 start*/
static void syna_dev_resume_work(struct work_struct *work)
{
	int retval;
	struct delayed_work *delayed_work;
	struct syna_tcm *tcm;
	...
	if (!work) {
		LOGE("Invalid work struct pointer\n");
		return;
	}

	delayed_work = container_of(work, struct delayed_work, work);
	if (!delayed_work) {
		LOGE("Invalid delayed_work struct pointer\n");
		return;
	}

	tcm = container_of(delayed_work, struct syna_tcm, resume_work);
	if (!tcm) {
		LOGE("Invalid tcm struct pointer\n");
		return;
	}

	if (!tcm->pdev) {
		LOGE("pdev field in tcm struct is NULL\n");
		return;
	}

	retval = syna_dev_resume(&tcm->pdev->dev);
	if (retval) {
		LOGE("Failed to resume syna device\n");
		return;
	}
...
/*pri LAX10-1016 modify by ocean 20240620 start*/
#if IS_ENABLED(CONFIG_CS_NOTIFIER)
static int syna_tcm_usb_notifier_callback(struct notifier_block *nb, unsigned long event, void *data)
{
	int retval;
	struct syna_tcm *tcm =
				container_of(nb, struct syna_tcm, usb_notifier);

	LOGI("charge notifier,event:%lu\n", event);
	if (event == USB_PLUG_IN) {
		tcm->usb_state = 1;
		if (atomic_read(&tcm->now_system_status) == 1) {
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev, 0X07, 1,RESP_IN_ATTN);
			if (retval < 0) {
				LOGE("Fail to set charge type fail\n");
			}
		...
		tcm->usb_state = 0;
		if (atomic_read(&tcm->now_system_status) == 1) {
			retval = syna_tcm_set_dynamic_config(tcm->tcm_dev, 0X07, 0,RESP_IN_ATTN);
			if (retval < 0) {
				LOGE("Fail to set charge type fail\n");
			}
			LOGI("Charge pull out!\n");
		}
	} else {
		LOGI("Unknown charging status!\n");
		return -EINVAL;
	}
	return 0;
}
#endif
/*pri LAX10-1016 modify by ocean 20240620 end*/

/*pri LAX10-1016 modify by ocean 20240619 start*/
static void fodcheck_func(struct work_struct *work)
{
	//int retval;
	struct delayed_work *delayed_work;
	struct syna_tcm *tcm = NULL;
	struct input_dev *input_dev = NULL;//tcm->input_dev;
	if (!work) {
		LOGE("Invalid work struct pointer\n");
		return;
	}
	...
	tcm = container_of(delayed_work, struct syna_tcm, fod_work);
	if (!tcm) {
		LOGE("Invalid tcm struct pointer\n");
		return;
	}

	input_dev = tcm->input_dev;
	if (!input_dev) {
		LOGE("Invalid input_dev struct pointer\n");
		return;
	}
...
/*pri LAX10-651 modify by ocean 20240611 start*/
/*pri LAX10-1016 modify by ocean 20240619 start*/
	cancel_delayed_work_sync(&tcm->fod_work);
/*pri LAX10-1016 modify by ocean 20240619 end*/
	cancel_delayed_work_sync(&tcm->resume_work);
	flush_workqueue(tcm->resume_queue);
	destroy_workqueue(tcm->resume_queue);
/*pri LAX10-651 modify by ocean 20240611 end*/
...
static int syna_dev_connect(struct syna_tcm *tcm)
{
	...
	case MODE_APPLICATION_FIRMWARE:
		retval = syna_dev_set_up_app_fw(tcm);
		if (retval < 0) {
			LOGI("Fail to set up application firmware\n");
			...
			LOGI("allocate and register to input device subsystem\n");
			retval = syna_dev_set_up_input_device(tcm);
			if (retval < 0) {
...
/*pri LAX10-651 modify by ocean 20240611 start*/
	tcm->resume_queue =
			create_singlethread_workqueue("syna_resume");
	INIT_DELAYED_WORK(&tcm->resume_work, syna_dev_resume_work);
/*pri LAX10-651 modify by ocean 20240611 end*/
/*pri LAX10-1016 modify by ocean 20240619 start*/
	INIT_DELAYED_WORK(&tcm->fod_work, fodcheck_func);
/*pri LAX10-1016 modify by ocean 20240619 end*/
...
#if 0
/*pri LAX10-1016 modify by ocean 20240620 start*/
static void screenstate_type_func(unsigned char state)
{
	int retval = 0;

	if (atomic_read(&tcm_core->now_system_status) != 1)
		return;

	switch (state) {
	case 0:    //0°
		if (tcm_core->fw_boot_status == 1) {
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 220, 0, RESP_IN_ATTN);
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 221, 0x05, RESP_IN_ATTN);
			LOGI("%s portrait screen!\n", __func__);
		} else if (tcm_core->fw_boot_status == 0) {
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 11, 0x2001, RESP_IN_ATTN);
			LOGI("%s portrait screen!\n", __func__);
		}
		if (retval < 0) {
			LOGE("Fail to set screenstate type fail\n");
		}
		break;
	case 1:   //90°
		if (tcm_core->fw_boot_status == 1) {
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 220, 1, RESP_IN_ATTN);
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 221, 0x0c, RESP_IN_ATTN);
			LOGI("%s Right landscape screen!\n", __func__);
		} else if (tcm_core->fw_boot_status == 0) {
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 11, 0x2301, RESP_IN_ATTN);
			LOGI("%s Right landscape screen!\n", __func__);
		}
		if (retval < 0) {
			LOGE("Fail to set screenstate type fail\n");
	...
	case 2:   //270°
		if (tcm_core->fw_boot_status == 1) {
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 220, 1, RESP_IN_ATTN);
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 221, 0x03, RESP_IN_ATTN);
			LOGI("%s Left landscape screen!\n", __func__);
		} else if (tcm_core->fw_boot_status == 0) {
			retval = syna_tcm_set_dynamic_config(tcm_core->tcm_dev, 11, 0x2101, RESP_IN_ATTN);
			LOGI("%s Left landscape screen!\n", __func__);
		}
		if (retval < 0) {
			LOGE("Fail to set screenstate type fail\n");
	...
	default:
		LOGE("Unsupported screen state: %d\n", state);
		break;
	}
}
/*pri LAX10-1016 modify by ocean 20240620 end*/
 ...
 */
#ifdef ENABLE_WAKEUP_GESTURE
extern void pri_common_node_register(char* name,void(*set)(unsigned char on_off));
#endif
static int syna_dev_probe(struct platform_device *pdev)
{
	...
	struct device *dev;
#endif
	pr_err("[synaptics_tcm2] syna_dev_probe++\n");
...
#if IS_ENABLED(CONFIG_CS_NOTIFIER)
	tcm->fb_notifier.notifier_call = cs_notifier_callback;
	retval = cs_panel_notifier_register(&tcm->fb_notifier);
	if (retval < 0) {
		LOGE("Fail to register cs notifier client\n");
		goto err_create_cdev;
	}

	tcm->sensor_notifier.notifier_call = cs_sensor_notifier_callback;
	retval = cs_sensor_notifier_register(&tcm->sensor_notifier);
	if (retval < 0) {
		LOGE("Fail to register cs sensor notifier client\n");
		goto err_create_cdev;
	}
#elif defined(ENABLE_DISP_NOTIFIER)
#if defined(USE_DRM_PANEL_NOTIFIER)
...
#else
	tcm->fb_notifier.notifier_call = syna_dev_fb_notifier_cb;
	retval = mtk_disp_notifier_register("synaptics_tcm", &tcm->fb_notifier);
	if (retval < 0) {
		LOGE("Fail to register FB notifier client\n");
		goto err_create_cdev;
	}
...
#ifdef ENABLE_WAKEUP_GESTURE
	tcm_core = tcm;
	pri_common_node_register("GESTURE", &syna_double_type_func);
	pri_common_node_register("FINGER", &syna_double_fod_func);
	atomic_set(&tcm->fod_figer_state, 0);
#endif

/*pri LAX10-1016 modify by ocean 20240620 start*/
	atomic_set(&tcm->now_system_status, 1);
	//pri_common_node_register("SCREENSTATE",&screenstate_type_func);
#if IS_ENABLED(CONFIG_CS_NOTIFIER)
	tcm->usb_notifier.notifier_call = syna_tcm_usb_notifier_callback;
	retval = cs_usb_notifier_register(&tcm->usb_notifier);
	if (retval < 0) {
		LOGE("Failed to register usb notifier client\n");
	}
#endif
/*pri LAX10-1016 modify by ocean 20240620 end*/
...
#endif
#if IS_ENABLED(CONFIG_CS_NOTIFIER)
	cs_panel_notifier_unregister(&tcm->fb_notifier);
#elif defined(ENABLE_DISP_NOTIFIER)
#if defined(USE_DRM_PANEL_NOTIFIER)
	if (active_panel)
		drm_panel_notifier_unregister(active_panel,
				&tcm->fb_notifier);
#else
	mtk_disp_notifier_unregister(&tcm->fb_notifier);
#endif
#endif

#if IS_ENABLED(CONFIG_CS_NOTIFIER)
	cs_usb_notifier_unregister(&tcm->usb_notifier);
#endif
...
{
	int retval;
	pr_err("[SYN]syna_dev_module_init++\n");
