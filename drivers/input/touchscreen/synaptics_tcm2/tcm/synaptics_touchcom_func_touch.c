...
/*
	for (i = 0; i < config_size && (i * 5 + 1) < sizeof(buffer) - 1; ++i) { 
		sprintf(buf_ptr, "0x%02X ", config_data[i]);
		buf_ptr += 4;
	}
	...
	if (i > 0) {
    	...
    	*buf_ptr = '\0'; // 如果没有数据被写入，确保buffer至少是空字符串
	}

	printk("[syna]Config data values: %s\n", buffer);*/

	while (idx < config_size) {
		code = config_data[idx++];
		//printk("[syna]Current code: 0x%02X\n", code);
		switch (code) {
		...
		case TOUCH_REPORT_KNOB_CALIB:
			...
			offset += bits;
			break;
		case TOUCH_REPORT_GRIP_INFO:
			bits = config_data[idx++];
			offset += bits;
			break;
		case TOUCH_REPORT_FEEL_INFO:
			bits = config_data[idx++];
			offset += bits;
			break;
		default:
