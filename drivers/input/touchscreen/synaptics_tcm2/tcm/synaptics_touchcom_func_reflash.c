		...
		struct tcm_reflash_data_blob *reflash_data)
{
	...
	LOGI("Image Config ID: %s, Device Config ID:: %s\n",
...
//			goto exit;
	...
	LOGI("Device firmware ID: %d, image build id: %d\n",
		...
		struct tcm_buffer *rd_data, unsigned int rd_delay_us)
{
	...
	LOGI("BOOT_CONFIG address: 0x%x, length: %d\n", addr, length);
		...
		struct tcm_buffer *rd_data, unsigned int rd_delay_us)
{
	...
	LOGI("CUSTOM_OTP address: 0x%x, length: %d\n", addr, length);
		...
		struct block_data *block, unsigned int wr_delay_us)
{
	...
	LOGI("Write data to %s - address: 0x%x, size: %d\n",
		...
		unsigned int erase_delay_ms)
{
	...
	LOGI("Page start = %d (0x%04x), Page count = %d (0x%04x)\n",
		...
		struct block_data *block, unsigned int delay_ms)
{
	...
	LOGI("Erase %s block - address: 0x%x, size: %d\n",
		AREA_ID_STR(block->id), flash_addr, size);
		...
		enum update_area type, unsigned int wait_delay_ms)
{
	...
	switch (type) {
	case UPDATE_FIRMWARE_CONFIG:
		LOGI("type is UPDATE_FIRMWARE_CONFIG\n");
		...
		break;
	 // fall through to UPDATE_CONFIG_ONLY is intentional
	case UPDATE_CONFIG_ONLY:
		LOGI("type is UPDATE_CONFIG_ONLY\n");
		block = &reflash_data->image_info.data[AREA_APP_CONFIG];
