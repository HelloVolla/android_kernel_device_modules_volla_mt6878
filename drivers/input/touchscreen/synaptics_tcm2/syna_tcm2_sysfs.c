...
/**
 * syna_sysfs_sensor_debug_show()
 *
 * Attribute to show if sensor is enabled.
 *
 * @param
 *    [ in] kobj:  an instance of kobj
 *    [ in] attr:  an instance of kobj attribute structure
 ...
 *    otherwise, negative value on error.
 */
static ssize_t syna_sysfs_sensor_debug_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int retval;
	unsigned int count;
...
/**
 * syna_sysfs_sensor_debug_store()
 *
 * Attribute to debug sensor.
 *
 * @param
 *    [ in] kobj:  an instance of kobj
 *    [ in] attr:  an instance of kobj attribute structure
 ...
 *    on success, return count; otherwise, return error code
 */
static ssize_t syna_sysfs_sensor_debug_store(struct kobject *kobj,
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	int retval = 0;
	unsigned int temp;
	...
	LOGI("temp = %d, aft_enabled = %d\n", temp, tcm->aft_enabled);

	if (tcm->aft_enabled != 1) {
		LOGE("anti fake touch is not enabled yet, exit!\n");
		goto exit;
	}
...
static struct kobj_attribute kobj_attr_sensor_debug =
	__ATTR(sensor_debug, 0664, syna_sysfs_sensor_debug_show, syna_sysfs_sensor_debug_store);
#endif
/**
