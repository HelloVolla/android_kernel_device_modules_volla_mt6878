...
*****************************************************************************/
#define FTS_DRIVER_PEN_NAME                 "ft8725_ts,pen"
...
*****************************************************************************/
struct fts_ts_data *ft8725_data;
...
*****************************************************************************/
void ft8725_msleep(unsigned long msecs)
{
    if (msecs > 20) {
        msleep(msecs);
    } else if (msecs > 0) {
...
int ft8725_check_cid(struct fts_ts_data *ts_data, u8 id_h)
{
    int i = 0;
...
*****************************************************************************/
int ft8725_wait_tp_to_valid(void)
{
    int ret = 0;
    int i = 0;
    u8 idh = 0xFF;
    struct fts_ts_data *ts_data = ft8725_data;
    u8 chip_idh = ts_data->ic_info.ids.chip_idh;

    for (i = 0; i < FTS_MAX_RETRIES_READID; i++) {
        ret = ft8725_read_reg(FTS_REG_CHIP_ID, &idh);
        if ((idh == chip_idh) || (ft8725_check_cid(ts_data, idh) == 0)) {
            FTS_INFO("TP Ready,Device ID:0x%02x", idh);
            return 0;
        }
        if ((i + 1) < FTS_MAX_RETRIES_READID) ft8725_msleep((i + 1) * 20);
    }
...
/*****************************************************************************
*  Name: ft8725_tp_state_recovery
*  Brief: Need execute this function when reset
*  Input:
*  Output:
*  Return:
*****************************************************************************/
void ft8725_tp_state_recovery(struct fts_ts_data *ts_data)
{
    ft8725_wait_tp_to_valid();
    ft8725_ex_mode_recovery(ts_data);
#if FTS_PSENSOR_EN
    if (ts_data->proximity_mode) {
        ft8725_proximity_recovery(ts_data);
        return;
    }
#endif
#if FTS_FOD_EN
...
#endif
    ft8725_gesture_recovery(ts_data);
}
...
{
    if (ts_data->fwdbg_support && value) ft8725_fwdbg_handle_reset(ts_data);

    return 0;
}

int ft8725_set_reset(struct fts_ts_data *ts_data, int value)
{
    int ret = 0;
...
int ft8725_reset_proc(struct fts_ts_data *ts_data, int force, int hdelayms)
{
    if (force || (!ts_data->fw_loading)) {
        ft8725_set_reset(ts_data, 0);
        ft8725_msleep(2);
        ft8725_set_reset(ts_data, 1);
        if (hdelayms) ft8725_msleep(hdelayms);
    } else {
...
void ft8725_irq_disable(void)
{
    unsigned long irqflags;

    FTS_FUNC_ENTER();
    spin_lock_irqsave(&ft8725_data->irq_lock, irqflags);

    if (!ft8725_data->irq_disabled) {
        disable_irq_nosync(ft8725_data->irq);
        ft8725_data->irq_disabled = true;
    }

    spin_unlock_irqrestore(&ft8725_data->irq_lock, irqflags);
    FTS_FUNC_EXIT();
}

void ft8725_irq_enable(void)
{
    ...
    if (ft8725_data->irq_disabled) {
        enable_irq(ft8725_data->irq);
        ft8725_data->irq_disabled = false;
    }
...
int ft8725_hid2std(int mode)
{
    int ret = 0;
    u8 buf[3] = {0xEB, 0xAA, 0x09};
    u8 val[3] = { 0 };

    if (ft8725_data->bus_type != BUS_TYPE_I2C)
        return 0;

    if (mode == 1) {
        /* Don't need delay */
        ret = ft8725_read(buf, 3, val, 3);
        if (ret < 0) {
            FTS_ERROR("send hid2std cmd failed");
            return ret;
        }
    } else {
        ret = ft8725_write(buf, 3);
        if (ret < 0) {
...
static int fts_read_bootid(struct fts_ts_data *ts_data, u8 *id)
{
    ...
    ret = ft8725_write(id_cmd, 2);
    if (ret < 0) {
        FTS_ERROR("start cmd write fail");
        return ret;
    }

    ft8725_msleep(FTS_CMD_START_DELAY);
    ...
    ret = ft8725_read(id_cmd, id_cmd_len, chip_id, 2);
...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    for (cnt = 0; cnt < FTS_MAX_RETRIES_READ_BOOTID; cnt++) {
        ft8725_reset_proc(ts_data, true, 0);
...
static void fts_fod_set_reg(int value)
{
    ...
    for (i = 0; i < FTS_MAX_RETRIES_WRITEREG; i++) {
        ft8725_read_reg(FTS_REG_FOD_MODE_EN, &regval);
        if (regval == fod_val)
            break;
        ft8725_write_reg(FTS_REG_FOD_MODE_EN, fod_val);
        ft8725_msleep(1);
    }
...
void ft8725_fod_enable(int enable)
{
    struct fts_ts_data *ts_data = ft8725_data;
...
static int fts_fod_readdata(struct fts_ts_data *ts_data)
{
    ...
    ret = ft8725_read(&fod_cmd, 1, fod_val, FTS_FOD_BUF_LEN);
    if (ret < 0) {
...
void ft8725_release_all_finger(void)
{
    struct fts_ts_data *ts_data = ft8725_data;
...
int ft8725_input_report_buffer(struct fts_ts_data *ts_data, u8 *report_buf)
{
    int ret = 0;
    int touch_etype = 0;
...
static int fts_read_touchdata_spi(struct fts_ts_data *ts_data, u8 *buf)
{
    ...
    ret = ft8725_read(&ts_data->touch_addr, 1, buf, ts_data->touch_size);

    if (((0xEF == buf[1]) && (0xEF == buf[2]) && (0xEF == buf[3]))
        || ((ret < 0) && (0xEF == buf[0]))) {
        ft8725_release_all_finger();
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
...
#if FTS_PSENSOR_EN
    if (ts_data->proximity_mode) {
        if (ft8725_proximity_readdata(ts_data) == FTS_RETVAL_IGNORE_TOUCHES)
            return TOUCH_IGNORE;
    }
#endif
    ...
    if (ts_data->suspended && ts_data->gesture_support) {
        if (ft8725_gesture_readdata(ts_data, touch_buf) == FTS_RETVAL_IGNORE_TOUCHES)
            return TOUCH_IGNORE;
    }
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    case TOUCH_FW_INIT:
        ft8725_release_all_finger();
        ft8725_tp_state_recovery(ts_data);
        break;
...
static irqreturn_t fts_irq_handler(int irq, void *data)
{
    ...
    if (ts_data->fhp_mode) {
        ft8725_fhp_irq_handler(ts_data);
        return IRQ_HANDLED;
    }

    ts_data->intr_jiffies = jiffies;
    ft8725_prc_queue_work(ts_data);
    ...
    if (ts_data->fwdbg_support) {
        ft8725_fwdbg_readdata(ts_data, ts_data->touch_buf);
    }
    return IRQ_HANDLED;
}
...
#else
#if (!FTS_CHIP_IDC)
    ret = ft8725_set_reset(ts_data, 0);
    if (ret) return ret;
#endif
    ft8725_msleep(2);
    ret = ft8725_set_reset(ts_data, 1);
    if (ret) return ret;
#endif /* FTS_POWER_SOURCE_CUST_EN */
...
static int fts_power_suspend(struct fts_ts_data *ts_data)
{
    ...
    ret = ft8725_write_reg(FTS_REG_POWER_MODE, FTS_REG_POWER_MODE_SLEEP);
    if (ret < 0) {
...
static int fts_power_resume(struct fts_ts_data *ts_data)
{
    ...
    ft8725_msleep(FTS_DELAY_RESUME_RESET);
#else
    FTS_ERROR("FTS_POWER_SOURCE_CUST_EN=0,FTS_POWER_SUSPEND_OFF_EN=1");
#endif /* FTS_POWER_SOURCE_CUST_EN */
    ...
    if (!ts_data->ic_info.is_incell) {
        ft8725_reset_proc(ts_data, false, FTS_DELAY_RESUME_RESET);
    }
#endif /* FTS_POWER_SUSPEND_OFF_EN */
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    ft8725_esdcheck_suspend(ts_data);
#if FTS_PSENSOR_EN
    if (ts_data->proximity_mode) {
        ft8725_proximity_suspend(ts_data);
    ...
    if (ts_data->gesture_support) {
        ft8725_gesture_suspend(ts_data);
        ts_data->need_work_in_suspend = true;
    }
...
static int fts_ts_resume(struct device *dev)
{
    ...
    if (ts_data->proximity_mode) {
        ft8725_wait_tp_to_valid();
        ft8725_proximity_resume(ts_data);
        ft8725_esdcheck_resume(ts_data);
        if (ts_data->gesture_support) ft8725_gesture_resume(ts_data);
        return 0;
    }
#endif
    ...
    ft8725_enter_normal_fw();
    if (ts_data->gesture_support) {
        ft8725_gesture_resume(ts_data);
    }
#if FTS_FOD_EN
    if (ts_data->fod_mode) {
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *v)
{
    ...
    if (ts_data && v) {
        ...
        if ((blank_enum[1] == blank_value) && (event_enum[1] == event)) {
            queue_work(ft8725_data->ts_workqueue, &ft8725_data->resume_work);
        } else if ((blank_enum[0] == blank_value) && (event_enum[0] == event)) {
            cancel_work_sync(&ft8725_data->resume_work);
...
int ft8725_ts_probe_entry(struct fts_ts_data *ts_data)
{
    int ret = 0;
    ...
    ret = ft8725_create_apk_debug_channel(ts_data);
    if (ret) {
        FTS_ERROR("create apk debug node fail");
    }

    ret = ft8725_create_sysfs(ts_data);
    if (ret) {
        FTS_ERROR("create sysfs node fail");
    }

    ret = ft8725_fwdbg_init(ts_data);
    if (ret) {
        FTS_ERROR("FwDebug init fail");
    }

    ret = ft8725_fhp_init(ts_data);
    if (ret) {
        FTS_ERROR("fhp init fail");
    }

    ret = ft8725_point_report_check_init(ts_data);
    if (ret) {
        FTS_ERROR("init point report check fail");
    }

    ret = ft8725_ex_mode_init(ts_data);
    if (ret) {
        FTS_ERROR("init glove/cover/charger fail");
    }

    ret = ft8725_gesture_init(ts_data);
    if (ret) {
        FTS_ERROR("init gesture fail");
    }

#if FTS_PSENSOR_EN
    ret = ft8725_proximity_init(ts_data);
    if (ret) {
    ...
    ret = ft8725_esdcheck_init(ts_data);
    if (ret) {
...
//prize added by huarui, fts support, 20190111-start
	...
	ft8725_read_reg(0xA6, &ctp_fw_version);
	sprintf(current_tp_info.chip,"FT8725,FW=0x%02x",ctp_fw_version);
    sprintf(current_tp_info.id,"chipid=0x%02x%02x",ts_data->ic_info.ids.chip_idh, ts_data->ic_info.ids.chip_idl);
    strcpy(current_tp_info.vendor,"Focaltech");
    sprintf(current_tp_info.more,"%d*%d",1080,2460);
	}
#endif
//prize added by huarui, fts support, 20190111-end
...
err_irq_req:
    ft8725_esdcheck_exit(ts_data);
#if FTS_PSENSOR_EN
    ft8725_proximity_exit(ts_data);
#endif
    ft8725_gesture_exit(ts_data);
    ft8725_ex_mode_exit(ts_data);
    ft8725_point_report_check_exit(ts_data);
    ft8725_fhp_exit(ts_data);
    ft8725_fwdbg_exit(ts_data);
    ft8725_remove_sysfs(ts_data);
    ft8725_release_apk_debug_channel(ts_data);
...
int ft8725_ts_remove_entry(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    cancel_work_sync(&ts_data->resume_work);
    fts_notifier_callback_exit(ts_data);
    free_irq(ts_data->irq, ts_data);
    ft8725_fwupg_exit(ts_data);
    ft8725_esdcheck_exit(ts_data);
#if FTS_PSENSOR_EN
    ft8725_proximity_exit(ts_data);
#endif
