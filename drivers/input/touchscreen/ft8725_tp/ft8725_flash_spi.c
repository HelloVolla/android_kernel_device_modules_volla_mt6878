...
struct upgrade_module ft8725_module_list[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, ft8725_fw_file, sizeof(ft8725_fw_file)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, ft8725_fw_file2, sizeof(ft8725_fw_file2)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, ft8725_fw_file3, sizeof(ft8725_fw_file3)},
};

struct upgrade_setting_nf ft8725_upgrade_setting_list[] = {
...
struct fts_upgrade *ft8725_fwupgrade;
...
static int fts_check_bootid(void)
{
    ...
    u8 cmd = 0;
    u8 id[2] = { 0 };
    struct fts_upgrade *upg = ft8725_fwupgrade;
...
static int fts_enter_into_boot(void)
{
    ...
    for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
        ...
        ft8725_reset_proc(upg->ts_data, true, 0);
        ...
        for (j = 0; j < FTS_READ_BOOT_ID_TIMEOUT; j++) {
            ...
            ret = ft8725_write(cmd, 1);
            if (ret >= 0) {
                ft8725_msleep(upg->setting_nf->delay_init);
...
static bool fts_check_fast_download(void)
{
    ...
    ret = ft8725_read_reg(0xdb, &value);
    if (ret < 0) {
        FTS_ERROR("read 0xdb fail");
        goto read_err;
    }

    ret = ft8725_read(cmd, 6, value2, 2);
    if (ret < 0) {
...
static int fts_dpram_write_pe(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = ft8725_write(&cmd[0], FTS_CMD_WRITE_LEN + packet_len);
        if (ret < 0) {
...
static int fts_dpram_write(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        cmd[2] = BYTE_OFF_8(addr);
        cmd[3] = BYTE_OFF_0(addr);
        ret = ft8725_write(&cmd[0], FTS_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
        if (ret < 0) {
        ...
        ret = ft8725_write(&cmd[0], 1 + packet_len);
        if (ret < 0) {
            FTS_ERROR("write fw to pram(%d) fail", i);
            goto write_pram_err;
        }
...
static int fts_ecc_cal_tp(u32 ecc_saddr, u32 ecc_len, u16 *ecc_value)
{
    ...
    ret = ft8725_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    if (upg->setting_nf->ecc_delay) {
        ft8725_msleep(upg->setting_nf->ecc_delay);
    } else {
        cmd[0] = FTS_ROMBOOT_CMD_ECC_FINISH;
        for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
            ret = ft8725_read(cmd, 1, value, 1);
            if (ret < 0) {
...
static int fts_fw_download(const u8 *buf, u32 len, bool need_reset)
{
    ...
    if (ft8725_esdcheck_is_running(upg->ts_data)) {
        ft8725_esdcheck_switch(upg->ts_data, DISABLE);
    ...
    if (esd_need_recovery) ft8725_esdcheck_switch(upg->ts_data, ENABLE);
    ret = 0;
err_fw_download:
    if (irq_need_recovery) ft8725_irq_enable();
...
int ft8725_enter_test_environment(bool test_state)
{
    ...
    ft8725_msleep(50);
    ret = ft8725_read_reg(FTS_REG_FW_MODE, &detach_flag);
...
static bool fts_check_fw_normal(void)
{
    ...
    for (i = 0; i < max_retries; i++) {
        ft8725_read_reg(FTS_REG_CHIP_ID, &val);
        if ((val == chip_id->chip_idh) || (ft8725_check_cid(upg->ts_data, val) == 0)) {
            FTS_INFO("TP Ready,Read ID=0x%02x", val);
#if (FTS_MULTI_FW_NUM > 1)
            ft8725_read_reg(FTS_REG_FW_MODE, &val);
            ...
            upg->ts_data->fw_is_running = false;
            ft8725_read_reg(FTS_CMD_READ_BOOT_STATE, &boot_state);
        ...
        if ((i + 1) < max_retries) ft8725_msleep((i + 1) * 20);
    }
...
int ft8725_enter_gesture_fw(void)
{
    ...
    if (fts_fw_resume(true, FW_GESTURE) == 0) {
        ft8725_tp_state_recovery(upg->ts_data);
        ft8725_read_reg(FTS_REG_FW_MODE, &fw_mode);
...
int ft8725_fw_recovery(void)
{
    ...
    ret = ft8725_read_reg(FTS_CMD_READ_BOOT_STATE, &boot_state);
    if (ret < 0) {
...
static void fts_fwload_work(struct work_struct *work)
{
    ...
    ft8725_read_reg(FTS_REG_CHIP_ID, &chip_id);
...
{
    int i = 0;
    struct upgrade_module *info = &ft8725_module_list[0];
...
{
    upg->fw = upg->module_info->ft8725_fw_file;
...
static void fts_fwupg_work(struct work_struct *work)
{
    ...
    if (ret < 0) {
        ...
        ft8725_msleep(50);
        ret = ft8725_read_reg(FTS_REG_CHIP_ID, &chip_id);
        FTS_INFO("read chip id:0x%02x", chip_id);
        ft8725_esdcheck_switch(upg->ts_data, ENABLE);
    }
}
...
{
    int i = 0;
    struct upgrade_setting_nf *setting = &ft8725_upgrade_setting_list[0];
    int setting_count =
        sizeof(ft8725_upgrade_setting_list) / sizeof(ft8725_upgrade_setting_list[0]);
    ...
    ft8725_fwupgrade = (struct fts_upgrade *)kzalloc(sizeof(*ft8725_fwupgrade), GFP_KERNEL);
    if (NULL == ft8725_fwupgrade) {
        FTS_ERROR("malloc memory for upgrade fail");
        return -ENOMEM;
    }

    if (1 == setting_count) {
        ft8725_fwupgrade->setting_nf = setting;
    } else {
        for (i = 0; i < setting_count; i++) {
            setting = &ft8725_upgrade_setting_list[i];
    ...
    if (NULL == ft8725_fwupgrade->setting_nf) {
        FTS_ERROR("no upgrade settings match, can't upgrade");
        kfree(ft8725_fwupgrade);
        ft8725_fwupgrade = NULL;
        return -ENODATA;
    }

    ft8725_esdcheck_switch(ts_data, DISABLE);
    ft8725_fwupgrade->ts_data = ts_data;
    INIT_WORK(&ft8725_fwupgrade->fwupg_work, fts_fwupg_work);
    INIT_WORK(&ft8725_fwupgrade->fwrecover_work, fts_fwrecover_work);
    INIT_WORK(&ft8725_fwupgrade->fwload_work, fts_fwload_work);
    queue_work(ts_data->ts_workqueue, &ft8725_fwupgrade->fwupg_work);
...
int ft8725_fwupg_exit(struct fts_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (ft8725_fwupgrade) {
        cancel_work_sync(&ft8725_fwupgrade->fwupg_work);
        cancel_work_sync(&ft8725_fwupgrade->fwrecover_work);
        cancel_work_sync(&ft8725_fwupgrade->fwload_work);
        if (ft8725_fwupgrade->fw_from_request) {
            vfree(ft8725_fwupgrade->fw);
            ft8725_fwupgrade->fw = NULL;
        }
