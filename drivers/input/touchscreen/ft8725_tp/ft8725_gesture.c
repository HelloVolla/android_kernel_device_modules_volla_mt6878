...
/* DRV added by wangwei1 20240809, gestuer node, start */
#if defined(CONFIG_PRI_COMMON_NODE)
#include "pri_common_node.h"
#endif
/* DRV modified by wangwei1 20240809, gesture mode, end */
/******************************************************************************
...
#define GESTURE_DOUBLECLICK                     0x26// 0x24 one tap  0x26 double
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ft8725_read_reg(FTS_REG_GESTURE_EN, &val);
...
/* DRV added by wangwei1 20240809, gestuer node, start */
#if defined(CONFIG_PRI_COMMON_NODE)
static struct fts_ts_data *ft8725_gesture_data;
static void ft8725_double_type_func(unsigned char on)
{
    if (ft8725_gesture_data->suspended) {
        FTS_INFO("In suspend,not operation gesture mode!");
        return;
    }

    mutex_lock(&ft8725_gesture_data->input_dev->mutex);
	if(1 == on){
        FTS_INFO("enable gesture");
        ft8725_gesture_data->gesture_support = ENABLE;
	}else if(0 == on){
        FTS_INFO("disable gesture");
        ft8725_gesture_data->gesture_support = DISABLE;
	}
    mutex_unlock(&ft8725_gesture_data->input_dev->mutex);
...
*         -Exx - error
*****************************************************************************/
int ft8725_gesture_readdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    int ret = 0;
    int i = 0;
    int index = 0;
    ...
    ret = ft8725_read_reg(FTS_REG_GESTURE_EN, &gesture_en);
    ...
    if ((ts_data->gesture_bmode == GESTURE_BM_TOUCH) && touch_buf &&
        ...
        ret = ft8725_read(&buf[2], 1, &buf[2], FTS_GESTURE_DATA_LEN - 2);
        if (ret < 0) {
...
void ft8725_gesture_recovery(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->gesture_support && ts_data->suspended) {
        ft8725_write_reg(0xD1, 0xFF);
        ft8725_write_reg(0xD2, 0xFF);
        ft8725_write_reg(0xD5, 0xFF);
        ft8725_write_reg(0xD6, 0xFF);
        ft8725_write_reg(0xD7, 0xFF);
        ft8725_write_reg(0xD8, 0xFF);
        ft8725_write_reg(FTS_REG_GESTURE_EN, ENABLE);
        ft8725_msleep(1);
        ft8725_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state != ENABLE) {
...
int ft8725_gesture_suspend(struct fts_ts_data *ts_data)
{
    ...
    for (i = 0; i < FTS_MAX_RETRIES_WRITEREG; i++) {
        ...
        ft8725_write_reg(0xD8, 0xFF);
        ft8725_write_reg(FTS_REG_GESTURE_EN, ENABLE);
        ft8725_write_reg(0xCF, 0x02);//0x00 one tap  0x02 double
        ft8725_msleep(1);
        ft8725_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == ENABLE)
            break;
...
int ft8725_gesture_resume(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    for (i = 0; i < FTS_MAX_RETRIES_WRITEREG; i++) {
        ft8725_write_reg(FTS_REG_GESTURE_EN, DISABLE);
        ft8725_msleep(1);
        ft8725_read_reg(FTS_REG_GESTURE_EN, &state);
        if (state == DISABLE)
            break;
...
/* DRV added by wangwei1 20240809, gestuer node, start */
#if defined(CONFIG_PRI_COMMON_NODE)
	ft8725_gesture_data = ts_data;
	pri_common_node_register("GESTURE", &ft8725_double_type_func);
#endif
/* DRV modified by wangwei1 20240809, gesture mode, end */
