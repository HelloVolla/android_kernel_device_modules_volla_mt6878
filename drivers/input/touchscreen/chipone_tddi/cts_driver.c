...
/* drv add by chenjiaxi, charger mode don't load tp driver, begin */
struct tag_bootmode {
	u32 size;
	u32 tag;
	u32 bootmode;
...
static unsigned int get_boot_mode_from_dts(void)
{
	...
	if (node)
		tags = (struct tag_bootmode *)of_get_property(node, "atag,boot", NULL);
	else
		printk("drv_charger: of_chosen not found\n");

	if (tags) {
		ret = tags->bootmode;
		printk("drv_charger: 'atag,boot' mode %d\n", ret);
	} else
		printk("drv_charger: 'atag,boot' not found\n");

	return ret;
}
/* drv add by chenjiaxi, charger mode don't load tp driver, end */
...
#endif
{
	...
	cts_info("%s\n", __func__);
	/* drv add by chenjiaxi, charger mode don't load tp driver, begin */
	if(get_boot_mode_from_dts() == 8) {
		printk("drv_charger: charger mode don't load tp driver.\n");
		g_charger_mode = true;
		return -1;
	} else {
		printk("drv_charger: normal mode.\n");
	}
	/* drv add by chenjiaxi, charger mode don't load tp driver, end */
...
#else
static void cts_driver_remove(struct spi_device *spi)
#endif
{
    struct chipone_ts_data *cts_data;
    int ret = 0;
