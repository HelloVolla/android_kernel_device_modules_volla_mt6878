...
//drv added by chenjiaxi, gesture/glove, begin
static struct cts_device *g_cts_dev;

#if IS_ENABLED(CONFIG_PRI_COMMON_NODE)
#ifdef CFG_CTS_GESTURE
static void drv_gesture_func(unsigned char on)
{
	if (1 == on) {
		cts_enable_gesture_wakeup(g_cts_dev);
		cts_info("%s open DOUBLE-TAP gesture\n", __func__);
	} else if (0 == on) {
		cts_disable_gesture_wakeup(g_cts_dev);
		cts_info("%s close DOUBLE-TAP gesture\n", __func__);
	}
}
#endif
#endif
...
/* change based on TP driver start */
...
#endif
//drv added by chenjiaxi, gesture/glove, end
...
#if IS_ENABLED(CONFIG_PRI_COMMON_NODE)
#ifdef CFG_CTS_GESTURE
	pri_common_node_register("GESTURE", &drv_gesture_func);
#endif
#endif
...
bool g_cts_is_gesture_wakeup_enabled(void)
{
	if (g_charger_mode)
		return false;
	else
		return g_cts_dev->rtdata.gesture_wakeup_enabled;
}
EXPORT_SYMBOL_GPL(g_cts_is_gesture_wakeup_enabled);
...
#ifdef CONFIG_CTS_GLOVE
                if (cts_is_glove_enabled(&cts_data->cts_dev))
...
/* drv add by chenjiaxi, disable esd check before firmware upgrading, begin */
#ifdef CONFIG_CTS_ESD_PROTECTION
    cts_disable_esd_protection(cts_data);
#endif
/* drv add by chenjiaxi, disable esd check before firmware upgrading, end */
