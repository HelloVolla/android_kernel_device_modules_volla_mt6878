...
static ssize_t tfa98xx_procfs_rpc_read(struct file *file,
	char __user *user_buf, size_t count,
	loff_t *ppos)
{
    struct i2c_client *i2c = file->private_data;
...
static ssize_t tfa98xx_procfs_rpc_send(struct file *file,
	const char __user *user_buf,
	size_t count, loff_t *ppos)
{
    struct i2c_client *i2c = file->private_data;
...
static ssize_t tfa98xx_cmd_rpc_read(struct i2c_client *i2c,
...
static ssize_t tfa98xx_cmd_rpc_send(struct i2c_client *i2c,
	char *buffer, size_t count)
{
	struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
	TfaFileDsc_t *msg_file = NULL;
	...
	memcpy(msg_file->data, buffer, count);
	pr_info("[0x%x] dsp_msg buffer:%p count: %d\n", i2c->addr, buffer, (int)count);
	...
	if (tfa98xx->tfa->is_probus_device) {
		...
		if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
			error = tfaContWriteFile(tfa98xx->tfa, msg_file, 0, 0); /* int vstep_idx, int vstep_msg_idx both 0 */
			if (error != Tfa98xx_Error_Ok) {
				pr_err("[0x%x] tfaContWriteFile error: %d\n", tfa98xx->i2c->addr, error);
				err = -EIO;
			}
		}
		else {
			error = tfa_dsp_msg(tfa98xx->tfa, msg_file->size, msg_file->data);
			if (error != Tfa98xx_Error_Ok) {
				pr_err("[0x%x] tfa_dsp_msg error: %d\n", tfa98xx->i2c->addr, error);
				err = -EIO;
			}
		}
	}
...
static ssize_t tfa98xx_attr_version_show(struct device *dev,
...
static DEVICE_ATTR(tfa_version, S_IRUGO|S_IWUSR, tfa98xx_attr_version_show, NULL);

static ssize_t tfa98xx_attr_lib_version_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret = 0;
	char bytes[27] = {0};

	pr_err("start to loop\n");
	sprintf(buf, "");
	do { // read lib version
		memset(&bytes[0], 0x00, sizeof(bytes));
		bytes[0] = 0x00;
		bytes[1] = 0x80;
		bytes[2] = 0xfe;
		/* send query command */
		ret = tfa98xx_cmd_rpc_send(tfa98xx_global[0]->i2c, bytes, 4);
		if (ret < 0) {
			pr_err("send query result cmd failed(%d)\n", ret);
			break;
		}
		/* read result */
		ret = tfa98xx_cmd_rpc_read(tfa98xx_global[0]->i2c, bytes, sizeof(bytes));
		if (ret < 0) {
			pr_err("read result failed(%d)\n", ret);
			break;
		}
        sprintf(buf, "version 1 is %u.%u.%u\n", bytes[0], bytes[1], bytes[2]);
	} while(0);

	pr_err("tfa98xx_version_show: %s\n", buf);
	return ret;
}

static DEVICE_ATTR(lib_version, S_IRUGO|S_IWUSR, tfa98xx_attr_lib_version_show, NULL);
...
//	itoa(cal_impedence[0],string,10);
	...
	pr_err("cal_impedence[0]=%d cal_impedence[1]=%d string=%s\n",cal_impedence[0],cal_impedence[1],string);
	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf,"%d:%d,%d:%d\n", 1, cal_impedence[0], 2, cal_impedence[1]);
...
static DEVICE_ATTR(cali_re, S_IRUGO|S_IWUSR,
	tfa98xx_calibrate_show, tfa98xx_calibrate_store);

static ssize_t tfa98xx_cali_f0_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret = 0;
	char bytes[27] = {0};
	int value;
	int f0_array[2];
	/* read algo status command */
	char memtrack[32] = { //SB5.5
		0x00, 0x80, 0x0b, 0x00, 0x00, 0x08,
		0x22, 0x00, 0x00, 0x22, 0x00, 0x01, // Re_t
		0x22, 0x00, 0x02, 0x22, 0x00, 0x03, // ic template
		0x22, 0x00, 0x1B, 0x22, 0x00, 0x1C, // req f0
		0x22, 0x00, 0x05, 0x22, 0x00, 0x06  // speak template
	};

	/* send memtrack command to aDSP */
	ret = tfa98xx_cmd_rpc_send(tfa98xx_global[0]->i2c, memtrack, sizeof(memtrack));
	if (ret < 0) {
		pr_err("send query memtrack failed(%d)\n", ret);
		sprintf(buf,"1:-1,2:-1\n");
		return ret;
	}

	/* query result after 3 second. */
	mdelay(1000);
	...
	do {
		...
		/* send query command */
		ret = tfa98xx_cmd_rpc_send(tfa98xx_global[0]->i2c, bytes, sizeof(bytes));
		if (ret < 0) {
			pr_err("send query result cmd failed(%d)\n", ret);
			sprintf(buf,"1:-1,2:-1\n");
			return ret;
		...
		value += bytes[20];
		f0_array[1] = value;
        ret = sprintf(buf, "Fres 1:%u,2:%u\n", f0_array[0], f0_array[1]);
		cal_f0[0] = f0_array[0];
		cal_f0[1] = f0_array[1];
	} while (0);

	pr_err("tfa98xx_f0_show:");
	pr_err("Fres 1:%u,2:%u\n", f0_array[0], f0_array[1]);
    pr_err("tfa98xx_f0_show end\n");
	return ret;
}

static ssize_t tfa98xx_cali_f0_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
...
static DEVICE_ATTR(cali_f0, S_IRUGO|S_IWUSR,
	tfa98xx_cali_f0_show, tfa98xx_cali_f0_store);

static ssize_t tfa98xx_bypass_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret = 0;

	ret = sprintf(buf, "%d\n", tfa98xx_bypass);
	pr_err("tfa98xx_bypass_show: %d\n", tfa98xx_bypass);
	return ret;
}

static ssize_t tfa98xx_bypass_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	int bypass = 0;
	...
	do { // set bypass
		...
		bytes[5] = bypass;
		/* send query command */
		ret = tfa98xx_cmd_rpc_send(tfa98xx_global[0]->i2c, bytes, 6);
		if (ret < 0) {
			pr_err("send query result cmd failed(%d)\n", ret);
			break;
		}
	} while(0);
	tfa98xx_bypass = bypass;
	pr_err("tfa98xx_bypass_store: %d\n", tfa98xx_bypass);
	return count;
}

static DEVICE_ATTR(bypass, S_IRUGO|S_IWUSR,
	tfa98xx_bypass_show, tfa98xx_bypass_store);
#endif
...
#endif
static const struct proc_ops tfa98xx_procfs_rpc_fops = {
	.proc_open = simple_open,
	.proc_read = tfa98xx_procfs_rpc_read,
	.proc_write = tfa98xx_procfs_rpc_send,
...
#endif
	tfa98xx->proc_dbg_dir = proc_mkdir(name, NULL);
	proc_create_data("rpc", S_IRUGO|S_IWUGO, tfa98xx->proc_dbg_dir,
		&tfa98xx_procfs_rpc_fops, i2c);
...
static void tfa98xx_debug_remove(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->proc_dbg_dir)
		proc_remove(tfa98xx->proc_dbg_dir);
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	dev_info(tfa98xx->dev, "tfa profile name %s\n", name);
...
static void tfa98xx_interrupt_enable_tfa2(struct tfa98xx *tfa98xx, bool enable)
{
	...
	if ((tfa98xx->rev & 0xff) == 0x74 || (tfa98xx->rev & 0xff) == 0x73) {
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	*/
	...
	dev_info(tfa98xx->dev, "tfa_cnt_get_app_name tfa98xx->fw.name %s\n", tfa98xx->fw.name);
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (mute == 1 && stream == 0) {
		 ...
		 * are deactivated
		 */
        pr_info("[enter]%s[0x%x]: state: %d, stream : %d\n", __func__, tfa98xx->i2c->addr, mute, stream);
...
static struct bin_attribute dev_attr_tfa_reg = {
	.attr = {
		.name = "reg",
		.mode = S_IWUSR,
	},
...
static ssize_t reg_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	dev_info(tfa98xx_global[0]->dev, "reg_show tfa98xx_global[0]\n");
	for (reg = 0x00; reg < 0x80; reg++) {
		ret = regmap_read(tfa98xx_global[0]->regmap, reg, &val);
		if (ret < 0) {
			dev_err(tfa98xx_global[0]->dev, "i2c error at reg 0x%x\n", reg);
			return -3;
		}
		else {
			len += snprintf(buf + len, PAGE_SIZE - len,
					"reg:0x%02x=0x%04x\n", reg, val & 0xffff);
			dev_info(tfa98xx_global[0]->dev, "reg:0x%02x=0x%04x\n", reg, val & 0xffff);
		}
	}
	dev_info(tfa98xx_global[0]->dev, "reg_show tfa98xx_global[0] ok\n");

	dev_info(tfa98xx_global[1]->dev, "reg_show tfa98xx_global[1]\n");
	for (reg = 0x00; reg < 0x80; reg++) {
		ret = regmap_read(tfa98xx_global[1]->regmap, reg, &val);
		if (ret < 0) {
			dev_err(tfa98xx_global[1]->dev, "i2c error at reg 0x%x\n", reg);
			return -3;
		}
		else {
			len += snprintf(buf + len, PAGE_SIZE - len,
					"reg:0x%02x=0x%04x\n", reg, val & 0xffff);
			dev_info(tfa98xx_global[1]->dev, "reg:0x%02x=0x%04x\n", reg, val & 0xffff);
		}
	}
	dev_info(tfa98xx_global[1]->dev, "reg_show tfa98xx_global[1] ok\n");
	return len;
}
...
static ssize_t force_calib_fsm_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	...
	/*dev[0]:7412 mOhms dev[1]:7369 mOhms*/
	ret = sprintf(buf,"dev[%d]:%d mOhms dev[%d]:%d mOhms\n", 1, cal_impedence[0], 2, cal_impedence[1]);
	pr_err("cal_impedence[0]=%d cal_impedence[1]=%d string=%s\n", cal_impedence[0], cal_impedence[1], buf);
	return ret;
}
	...
	const char *buf, size_t count)
{
	...
	pr_err("enter cali_re_store buf:%s", buf);
	pr_err("ok");
	return count;
}
...
static struct kobj_attribute dev_attr_reg = __ATTR_RO(reg);
	...
	const struct i2c_device_id *id)
{
	...
	ret = device_create_bin_file(&i2c->dev, &dev_attr_tfa_reg);
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs files\n");
	...
	ret = device_create_file(&i2c->dev,&dev_attr_tfa_version);
	if (ret) {
		dev_err(&i2c->dev,"failed to create device file for version\n");
		return ret;
	}

	ret = device_create_file(&i2c->dev,&dev_attr_lib_version);
	if (ret) {
		dev_err(&i2c->dev,"failed to create device file for lib version\n");
		return ret;
	}

	ret = device_create_file(&i2c->dev,&dev_attr_bypass);
	if (ret) {
		dev_err(&i2c->dev,"failed to create device file for bypass\n");
		return ret;
	}

#ifdef TFA98xx_calibrate
	ret = device_create_file(&i2c->dev,&dev_attr_cali_re);
	if (ret) {
		dev_err(&i2c->dev,"failed to create device file for cali_re\n");
		return ret;
	}
	ret = device_create_file(&i2c->dev,&dev_attr_cali_f0);
	if (ret) {
		dev_err(&i2c->dev,"failed to create device file for cali_f0\n");
		return ret;
	}
#endif
...
/* prize added for save smart PA cali_re value to NV storage start */
	...
	pr_info("aw883xx driver create fsm_drv_kobj :%p", fsm_drv_kobj);
	if (fsm_drv_kobj) {
		ret = sysfs_create_group(fsm_drv_kobj, &fsm_drv_attr_group);
		pr_info("aw883xx driver create sysfs_create_file ret:%d", ret);
	}
/* prize added for save smart PA cali_re value to NV storage end */
...
/* prize added for save smart PA cali_re value to NV storage end */
	...
	cancel_delayed_work_sync(&tfa98xx->unmute_work);
#endif
	device_remove_bin_file(&i2c->dev, &dev_attr_tfa_reg);
