...
//drv mod by pzp, use_ext_gpio 20250509 start	
#if IS_ENABLED(CONFIG_AW95016)
#include "../../../../drivers/misc/mediatek/pri/aw95016/aw95016.h"
#endif
//drv mod by pzp, use_ext_gpio 20250509 end
/******************************************************
 *
 * Marco
 *
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	ucontrol->value.enumerated.item[0] = aw883xx_dev_get_profile_index(aw883xx->aw_pa);
	//drv mod by pzp, fix ALSA control type mismatch 20250113 end
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	/* check value valid */
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	ret = aw883xx_dev_check_profile_index(aw883xx->aw_pa, ucontrol->value.enumerated.item[0]);
	if (ret) {
	...
	if (cur_index == ucontrol->value.enumerated.item[0]) {
	...
	mutex_lock(&aw883xx->lock);
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	aw883xx_dev_set_profile_index(aw883xx->aw_pa, ucontrol->value.enumerated.item[0]);
	//drv mod by pzp, fix ALSA control type mismatch 20250113 end
	...
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	aw_dev_info(codec->dev, "profile id %d", ucontrol->value.enumerated.item[0]);
	//drv mod by pzp, fix ALSA control type mismatch 20250113 end
	return 1;
}
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	ucontrol->value.enumerated.item[0] = aw883xx->allow_pw;
	//drv mod by pzp, fix ALSA control type mismatch 20250113 end
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	aw_dev_info(codec->dev, "set value:%d", ucontrol->value.enumerated.item[0]);

	if (ucontrol->value.enumerated.item[0] == aw883xx->allow_pw) {
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	ucontrol->value.enumerated.item[0] = monitor_desc->monitor_cfg.monitor_switch;
	//drv mod by pzp, fix ALSA control type mismatch 20250113 end
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	//drv mod by pzp, fix ALSA control type mismatch 20250113 start
	aw_dev_info(codec->dev, "set monitor_switch:%d", ucontrol->value.enumerated.item[0]);
...
static int aw883xx_gpio_request(struct aw883xx *aw883xx)
{
	...
	if (!aw883xx->use_ext_gpio) {
		...
		aw_dev_info(aw883xx->dev, "using ext-gpio mode");
		/* 在use_ext_gpio模式下，如果也配置了标准reset-gpio，也请求它 */
		if (gpio_is_valid(aw883xx->reset_gpio)) {
			ret = devm_gpio_request_one(aw883xx->dev, aw883xx->reset_gpio,
				GPIOF_OUT_INIT_LOW, "aw883xx_rst");
			if (ret) {
				aw_dev_err(aw883xx->dev, "rst request failed in ext-gpio mode");
				return ret;
			}
			aw_dev_info(aw883xx->dev, "standard reset_gpio also requested in ext-gpio mode");
		}
	}
	//drv mod by pzp, use_ext_gpio 20250509 end
...
static int aw883xx_parse_gpio_dt(struct aw883xx *aw883xx)
{
	...
	//drv mod by pzp, use_ext_gpio 20250509 start
	ret = of_property_read_u32(np, "use-ext-gpio", &aw883xx->use_ext_gpio);
	if (ret) {
		aw_dev_info(aw883xx->dev, "no use-ext-gpio defined, use standard gpio");
		aw883xx->use_ext_gpio = 0;
	}
	
	if (aw883xx->use_ext_gpio) {
		ret = of_property_read_u32(np, "ext-gpio-idx", &aw883xx->ext_gpio_idx);
		if (ret) {
			aw_dev_err(aw883xx->dev, "no ext-gpio-idx defined");
			//return -EINVAL;
		}
		aw_dev_info(aw883xx->dev, "using ext-gpio-idx: %d", aw883xx->ext_gpio_idx);
		
		/* 在use_ext_gpio模式下，也兼容解析reset-gpio */
		aw883xx->reset_gpio = of_get_named_gpio(np, "reset-gpio", 0);
		if (aw883xx->reset_gpio < 0) {
			aw_dev_info(aw883xx->dev, "no standard reset gpio in ext-gpio mode");
		} else {
			aw_dev_info(aw883xx->dev, "standard reset gpio also available in ext-gpio mode");
		}
	} else {
		aw883xx->reset_gpio = of_get_named_gpio(np, "reset-gpio", 0);
		if (aw883xx->reset_gpio < 0) {
...
static int aw883xx_hw_reset(struct aw883xx *aw883xx)
{
	...
	if (aw883xx->use_ext_gpio) {
#if IS_ENABLED(CONFIG_AW95016)
		pri_aw95016a_gpio_set(aw883xx->ext_gpio_idx, 1, 0);
		usleep_range(AW_1000_US, AW_1000_US + 10);
		pri_aw95016a_gpio_set(aw883xx->ext_gpio_idx, 1, 1);
		usleep_range(AW_1000_US, AW_1000_US + 10);
#endif
		aw_dev_info(aw883xx->dev, "ext gpio reset completed");
		
		/* 在use_ext_gpio模式下，如果也配置了标准reset-gpio，同时执行标准GPIO复位 */
		if (gpio_is_valid(aw883xx->reset_gpio)) {
			gpio_set_value_cansleep(aw883xx->reset_gpio, 0);
			usleep_range(AW_1000_US, AW_1000_US + 10);
			gpio_set_value_cansleep(aw883xx->reset_gpio, 1);
			usleep_range(AW_1000_US, AW_1000_US + 10);
			aw_dev_info(aw883xx->dev, "standard gpio reset also completed in ext-gpio mode");
		}
	} else {
		if (gpio_is_valid(aw883xx->reset_gpio)) {
			gpio_set_value_cansleep(aw883xx->reset_gpio, 0);
			...
			aw_dev_info(aw883xx->dev, "use standard gpio reset success");
		} else {
